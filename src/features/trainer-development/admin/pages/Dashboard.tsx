...
const maxMonthly = Math.max(
  ...monthlyTrainingTrend.map(m => Math.max(m.offline, m.online, m.hybrid))
);
...
const QUICK_ACTIONS = [
  ...
  },
  {
    ...
    icon: 'sitemap',
    path: tdmUrls.admin.competencyMapping,
  },
    ...
    >
      ...
      <div className="tdm-charts-row">
        ...
        <FormCard
          ...
          subtitle="Offline vs Online vs Hybrid"
        >
            ...
            {monthlyTrainingTrend.map(m => (
              <div key={m.month} className="tdm-trend-col">
                <div className="tdm-trend-bars">
                  <div
                    className="tdm-trend-bar"
                    style={{
                      height: `${(m.offline / maxMonthly) * 100}px`,
                      background: '#3b82f6',
                    }}
                  />
                  <div
                    className="tdm-trend-bar"
                    style={{
                      height: `${(m.online / maxMonthly) * 100}px`,
                      background: '#8b5cf6',
                    }}
                  />
                  <div
                    className="tdm-trend-bar"
                    style={{
                      height: `${(m.hybrid / maxMonthly) * 100}px`,
                      background: '#10b981',
                    }}
                  />
                </div>
          ...
          <div className="tdm-quick-actions">
            {QUICK_ACTIONS.map(a => (
              <button
                key={a.label}
                type="button"
                className="tdm-quick-action-btn"
            ...
            <button
              type="button"
              className="tdm-quick-action-btn"
              onClick={() => navigate(tdmUrls.admin.attendance)}
            >
                ...
                {trainingPrograms.slice(0, 8).map(t => (
                  <tr key={t.id}>
                    ...
                    <td style={{ color: '#6b7280', fontSize: '0.75rem' }}>
                      {t.trainer.split(' ').slice(0, 2).join(' ')}
                    </td>
                    <td>
                    ...
                    <td>
                      <span
                        className={`tdm-status-pill ${t.status.toLowerCase()}`}
                      >
                        {t.status}
                      </span>
                    </td>
            ...
            {approvalRequests
              ...
              .map(a => (
                ...
                >
                  ...
                  >
                    {a.type} • {a.requestedDate}
                  </p>
                  <p style={{ fontSize: '0.688rem', color: '#6b7280' }}>
                    Approver: <strong>{a.currentApprover}</strong>
                  </p>
                  {a.amount && (
                    <p
                      style={{
