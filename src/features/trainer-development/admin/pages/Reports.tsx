...
import { certificates, trainers, trainingAttendance, trainingPrograms } from '../../mocks';
import { tdmUrls } from '../../urls';

type ReportTab = 'master' | 'trainer' | 'attendance' | 'certificate' | 'budget' | 'audit';
        ...
        <div style={{ marginTop: '1.5rem' }}>
          {activeTab === 'master' && (
            <>
              ...
              <Table
                headers={['#', 'Programme Title', 'Category', 'Mode', 'Start Date', 'End Date', 'Trainer', 'Status']}
                rows={trainingPrograms.map((p, i) => [
                  String(i + 1), p.title, p.category, p.mode,
                  p.startDate, p.endDate, p.trainer, p.status,
                ])}
              />
            </>
          )}
          ...
          {activeTab === 'trainer' && (
            <>
              ...
              <Table
                headers={['#', 'Trainer Name', 'Department', 'Type', 'Sessions', 'Rating', 'Certifications']}
                rows={trainers.map((t, i) => [
                  String(i + 1), t.name, t.department, t.trainerType,
                  String(t.totalSessions), `${t.rating.toFixed(1)} / 5.0`, String(t.certifications.length),
                ])}
              />
            </>
          )}
          ...
          {activeTab === 'attendance' && (
            <>
              ...
              <Table
                headers={['#', 'Participant', 'Department', 'Training Programme', 'Date', 'Punch In', 'Punch Out', 'Status']}
                rows={trainingAttendance.map((a, i) => [
                  String(i + 1), a.participantName, a.department, a.trainingTitle,
                  a.date, a.punchIn, a.punchOut, a.status,
                ])}
              />
            </>
          )}
          ...
          {activeTab === 'certificate' && (
            <>
              ...
              <Table
                headers={['#', 'Certificate No', 'Participant', 'Programme', 'Issue Date', 'Type', 'Status']}
                rows={certificates.map((c, i) => [
                  String(i + 1), c.certificateNo, c.participantName, c.trainingTitle,
                  c.issueDate, c.type, c.status,
                ])}
              />
            </>
          )}
          ...
          {activeTab === 'budget' && (
            <>
              ...
              <Table
                headers={['#', 'Training Programme', 'Category', 'Mode', 'Estimated Budget', 'Approval Status', 'Approved By']}
                rows={trainingPrograms.filter(p => p.budget && p.budget > 0).map((p, i) => [
                  String(i + 1), p.title, p.category, p.mode,
                  `₹${p.budget.toLocaleString()}`, p.approvalStatus, p.approvedBy || '—',
                ])}
              />
            </>
          )}
          ...
          {activeTab === 'audit' && (
            <>
              ...
              <Table
                ...
                rows={[
                  ['1', '26 Jun 2024 10:15', 'Approved Budget', 'Finance Officer', 'Budget Approval', 'REQ-2024-001', 'Approved ₹45,000 for Python Workshop'],
                  ['2', '25 Jun 2024 14:30', 'Generated Certificates', 'Admin', 'Certificate Mgmt', 'AI FDP', 'Bulk generated 45 certificates'],
                  ['3', '25 Jun 2024 11:00', 'Registered Trainer', 'Admin', 'Trainer Master', 'Dr. Ramesh Kumar', 'New external trainer added'],
                  ['4', '24 Jun 2024 16:00', 'Scheduled Session', 'Training Coord', 'Training Schedule', 'SES-2024-012', 'Session rescheduled to 25 Jun'],
                  ['5', '23 Jun 2024 10:30', 'Feedback Exported', 'Admin', 'Feedback Mgmt', 'Leadership Workshop', 'Exported feedback to Excel'],
                  ['6', '22 Jun 2024 08:45', 'Attendance Marked', 'System', 'Attendance Mgmt', 'Cloud Computing', 'Auto-sync from QR scans completed'],
                ]}
              />
            </>
          )}
