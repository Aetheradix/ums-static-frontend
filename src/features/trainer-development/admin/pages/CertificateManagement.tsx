              ...
              cell: item => (
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: '#6b7280' }}>
                    {item.participantId} • {item.department}
                  </span>
                </div>
              ),
            },
            ...
            <div className="flex flex-col gap-4 w-full lg:w-1/3 border-r pr-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  ...
                  value={form.title || 'Certificate of Completion'}
                  onChange={e =>
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  className="border p-2 rounded text-sm w-full"
                  value={form.subheading || 'This is to certify that'}
                  onChange={e =>
                    setForm((f: any) => ({ ...f, subheading: e.target.value }))
                  }
                />
              </div>
              <div className="flex flex-col gap-1">
              ...
              <div className="flex flex-col gap-1">
                ...
                <input
                  className="border p-2 rounded text-sm w-full"
                  value={form.signature || 'Authorized Signatory'}
                  onChange={e =>
                    setForm((f: any) => ({ ...f, signature: e.target.value }))
                  }
                />
              </div>
              <div
            ...
            <div className="w-full lg:w-2/3 flex justify-center items-center bg-gray-50 border-2 border-dashed border-gray-300 rounded-lg p-4 min-h-[400px]">
              <div style={{ width: 520, height: 358, position: 'relative' }}>
                <div
                  ...
                  className="bg-white shadow-xl w-[800px] h-[550px] border-[10px] border-indigo-900 p-12 flex flex-col items-center justify-center text-center"
                >
                  ...
                  <p className="text-xl italic text-gray-600 mb-6">
                    {form.subheading || 'This is to certify that'}
                  </p>
                  ...
                  <div className="flex justify-between w-full mt-auto px-12 border-t pt-8 text-sm font-semibold">
                    ...
                    <div className="text-center">
                      <div className="text-blue-700 font-cursive italic text-xl -mt-4">
                        {form.signature || 'Authorized Signatory'}
                      </div>
                      <div className="border-t border-black mt-2 pt-1">
          ...
          <>
            <FormGrid columns={1}>
              <DropDownList
                ...
                value={form.program ?? ''}
                onChange={v =>
                  setForm((f: any) => ({ ...f, program: v as string }))
                }
              />
              <DropDownList
                label="Select Template"
                data={[
                  { name: 'Standard Completion', value: 'standard' },
                  { name: 'Workshop Participation', value: 'workshop' },
                ]}
                textField="name"
                optionValue="value"
                value={form.template ?? 'standard'}
                onChange={v =>
                  setForm((f: any) => ({ ...f, template: v as string }))
                }
              />
              <div
                style={{
              ...
              >
                <p
                  style={{ margin: 0, fontSize: '0.875rem', color: '#4338ca' }}
                >
                  <strong>Note:</strong> Certificates will only be generated for
                  participants with 'Present' attendance and passing assessment
          ...
          <>
            <FormGrid columns={1}>
              <TextBox
                ...
                onChange={v => setForm((f: any) => ({ ...f, participant: v }))}
                required
              />
              <DropDownList
                label="Select Training Programme"
              ...
              >
                <DropDownList
                  ...
                  value={form.type ?? ''}
                  onChange={v =>
                    setForm((f: any) => ({ ...f, type: v as string }))
                  }
                  required
                />
                <TextBox
                  ...
                  type="date"
                  value={form.expiry ?? ''}
                  onChange={v => setForm((f: any) => ({ ...f, expiry: v }))}
                />
              </div>
              <div style={{ marginTop: '1rem' }}>
                ...
                <label
                  style={{
                    ...
                    padding: '2rem',
                    textAlign: 'center',
                    background: form.file ? '#eef2ff' : '#f9fafb',
                    cursor: 'pointer',
                    borderColor: form.file ? '#6366f1' : '#d1d5db',
                  }}
                >
                  <input
                    type="file"
                    ...
                    onChange={e => {
                      ...
                      if (file)
                        setForm((f: any) => ({ ...f, file: file.name }));
                    }}
                  />
                  {form.file ? (
                    <>
