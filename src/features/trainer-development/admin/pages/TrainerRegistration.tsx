...
const STATUS_OPTIONS = [
  ...
  { name: 'On Deputation', value: 'On Deputation' },
];
...
export default function TrainerRegistration() {
  ...
  const handleSave = () => {
    ...
    if (popup.mode === 'create') {
      setData(prev => [
        {
          ...form,
          id: String(Date.now()),
          trainerId: `TDM-TR-${Math.floor(Math.random() * 1000)}`,
          rating: 0,
          totalSessions: 0,
          specialization: [],
          skills: [],
      ...
      setData(prev =>
        prev.map(d =>
          d.id === (popup as any).item.id ? ({ ...d, ...form } as Trainer) : d
        )
      );
      ...
      >
        ...
        <FormGrid columns={3}>
          ...
          <DropDownList
            ...
            onChange={v => setForm(f => ({ ...f, trainerType: v as any }))}
            disabled={isReadOnly}
          />
          <DropDownList
            ...
            onChange={v => setForm(f => ({ ...f, employmentType: v as any }))}
            disabled={isReadOnly}
          />
          <DropDownList
          ...
          <TextBox
            ...
            onChange={v => setForm(f => ({ ...f, experience: Number(v) }))}
            readOnly={isReadOnly}
          />
          <TextBox
            label="Industry Experience (Years)"
            type="number"
            value={form.industryExperience?.toString() ?? ''}
            onChange={v =>
              setForm(f => ({ ...f, industryExperience: Number(v) }))
            }
            readOnly={isReadOnly}
          />
          <DropDownList
        ...
        {!isReadOnly && (
          <div className="flex justify-end gap-3 mt-6">
            ...
            <Button
              label={
                popup.mode === 'create' ? 'Register Trainer' : 'Save Changes'
              }
              variant="primary"
              icon="check"
              onClick={handleSave}
