...
type PopupState =
  | { mode: 'closed' }
  | { mode: 'create' }
  | { mode: 'edit'; item: TrainingProgram }
  | { mode: 'view'; item: TrainingProgram };
const EMPTY: Partial<TrainingProgram> = {
  title: '',
  category: 'FDP',
  type: 'FDP',
  mode: 'Offline',
      ...
      >
        <FormGrid columns={2}>
          ...
          <DropDownList
            ...
            value={form.type}
            onChange={v =>
              setForm(f => ({ ...f, type: v as TrainingProgram['type'] }))
            }
            disabled={isReadOnly}
          />
          <DropDownList
            ...
            value={form.mode}
            onChange={v =>
              setForm(f => ({ ...f, mode: v as TrainingProgram['mode'] }))
            }
            disabled={isReadOnly}
          />
          <TextBox
          ...
          <TextBox
            ...
            onChange={v => setForm(f => ({ ...f, budget: Number(v) }))}
            readOnly={isReadOnly}
          />
        </FormGrid>
        {!isReadOnly && (
