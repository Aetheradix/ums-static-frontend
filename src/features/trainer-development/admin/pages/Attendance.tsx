      ...
      >
          ...
          >
            ...
            >
              <span
                ...
                style={{ fontSize: '5rem', color: '#9ca3af' }}
              >
                qr_code_2
              </span>
            </div>
            <p style={{ textAlign: 'center', color: '#4b5563' }}>
              Scan this QR code from the UMS App to register your attendance for
              the current session.
            </p>
          </div>
        )}
        {(popup.mode === 'manual' || popup.mode === 'edit') && (
          <>
            <FormGrid columns={1}>
              <TextBox
                label="Participant ID / Name"
                value={form.participantName ?? ''}
                onChange={v =>
                  setForm((f: any) => ({ ...f, participantName: v }))
                }
                required
              />
              <DropDownList
                ...
                onChange={v =>
                  setForm((f: any) => ({ ...f, status: v as string }))
                }
              />
              <TextBox
                label="Punch In Time"
                type="time"
                value={form.punchIn ?? ''}
                onChange={v => setForm((f: any) => ({ ...f, punchIn: v }))}
              />
              <TextBox
                label="Remarks"
                value={form.remarks ?? ''}
                onChange={v => setForm((f: any) => ({ ...f, remarks: v }))}
              />
            </FormGrid>
            <div
              style={{
