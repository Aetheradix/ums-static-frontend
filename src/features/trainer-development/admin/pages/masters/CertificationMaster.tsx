...
import {
  ...
  certificationMasters as initialData,
} from '../../../mocks';
...
type PopupState =
  | { mode: 'closed' }
  | { mode: 'create' }
  | { mode: 'edit'; item: CertificationMaster }
  | { mode: 'view'; item: CertificationMaster };

const EMPTY: Partial<CertificationMaster> = {
  code: '',
  name: '',
  validityMonths: 12,
...
export default function CertificationMasterPage() {
  ...
  const handleSave = () => {
    ...
    if (popup.mode === 'create') {
      setData(prev => [
        ...prev,
        { ...form, id: String(Date.now()) } as CertificationMaster,
      ]);
      ...
      >
        <FormGrid columns={2}>
          ...
          <TextBox
            ...
            type="number"
            value={form.validityMonths?.toString() ?? ''}
            onChange={v => setForm(f => ({ ...f, validityMonths: Number(v) }))}
            readOnly={isReadOnly}
          />
          <DropDownList
            label="Template Type"
            ...
            onChange={v => setForm(f => ({ ...f, templateType: v as any }))}
          />
          <DropDownList
            label="Status"
            data={STATUS_OPTIONS}
