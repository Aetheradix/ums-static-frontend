...
type PopupState = { mode: 'closed' } | { mode: 'create' } | { mode: 'edit'; item: Competency } | { mode: 'view'; item: Competency };

const EMPTY: Partial<Competency> = { code: '', name: '', category: 'Technical', description: '', levels: 5, applicableTo: [], status: 'Active' };
...
export default function CompetencyMaster() {
  ...
  const handleSave = () => {
    ...
    if (popup.mode === 'create') {
      setData(prev => [...prev, { ...form, id: String(Date.now()), applicableTo: form.applicableTo?.length ? form.applicableTo : ['All Departments'] } as Competency]);
      ToastService.success('Competency created.');
    } else if (popup.mode === 'edit') {
      setData(prev => prev.map(d => d.id === (popup as any).item.id ? { ...d, ...form } as Competency : d));
    ...
    >
      <FormCard>
        <GridPanel
          ...
          toolbar={<Button label="Add Competency" icon="plus" variant="primary" onClick={() => { setForm(EMPTY); setPopup({ mode: 'create' }); }} />}
          searchBox
        />
      </FormCard>
      ...
      <FormPopup
        ...
        onHide={close}
        title={popup.mode === 'create' ? 'Add Competency' : popup.mode === 'edit' ? 'Edit Competency' : 'View Competency'}
      >
        <FormGrid columns={2}>
          <TextBox label="Competency Code" value={form.code ?? ''} onChange={v => setForm(f => ({ ...f, code: v }))} required readOnly={isReadOnly} />
          <TextBox label="Competency Name" value={form.name ?? ''} onChange={v => setForm(f => ({ ...f, name: v }))} required readOnly={isReadOnly} />
          <DropDownList label="Category" data={CATEGORY_OPTIONS} textField="name" optionValue="value" value={form.category} onChange={v => setForm(f => ({ ...f, category: v as any }))} />
          <TextBox label="Levels" type="number" value={form.levels?.toString() ?? ''} onChange={v => setForm(f => ({ ...f, levels: Number(v) }))} readOnly={isReadOnly} />
