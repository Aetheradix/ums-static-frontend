...
type PopupState = { mode: 'closed' } | { mode: 'view'; item: any } | { mode: 'apply'; item: any };

export default function ApplyTrainingPage() {
  const [data] = useState(trainingPrograms.filter(t => t.status === 'Planned' || t.status === 'Scheduled'));
  ...
  return (
    ...
    >
      <FormCard>
        <GridPanel
          ...
          columns={[
            ...
            {
              field: 'seats', header: 'Availability',
              cell: (item) => (
                <div style={{ fontSize: '0.813rem' }}>
                  <span style={{ fontWeight: 600, color: item.maxParticipants - item.registeredCount < 5 ? '#ef4444' : '#10b981' }}>
                    {item.maxParticipants - item.registeredCount} Seats Left
                  </span>
                  <div style={{ fontSize: '0.75rem', color: '#6b7280' }}>Max: {item.maxParticipants}</div>
                </div>
              )
            },
            {
              ...
              cell: (item) => (
                <div style={{ display: 'flex', gap: '0.375rem' }}>
                  ...
                  <Button size="small" icon="check" variant="primary" label="Apply" onClick={() => setPopup({ mode: 'apply', item })} />
                </div>
              ),
            },
          ]}
      ...
      <FormPopup
        ...
        onHide={close}
        title={popup.mode === 'view' ? 'Programme Details' : popup.mode === 'apply' ? 'Confirm Application' : ''}
        size="default"
      >
        {popup.mode === 'view' && (
          <FormGrid columns={1}>
            ...
            <div>
              ...
              <p style={{ margin: 0 }}>Join this comprehensive training programme to enhance your skills in {popup.item.category}.</p>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              <div>
                <p style={{ fontSize: '0.75rem', color: '#6b7280', margin: '0 0 0.25rem 0' }}>Mode</p>
        ...
        {popup.mode === 'apply' && (
          <div style={{ textAlign: 'center', padding: '1rem' }}>
            ...
            <h3 style={{ margin: '0 0 0.5rem 0' }}>Apply for {popup.item.title}?</h3>
            <p style={{ color: '#6b7280', fontSize: '0.875rem' }}>Your request will be sent to the reporting manager for approval.</p>
            <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', marginTop: '2rem' }}>
              <Button label="Cancel" variant="outlined" onClick={close} />
              <Button label="Confirm Application" variant="primary" icon="check" onClick={close} />
            </div>
          </div>
        )}
      </FormPopup>
