...
export default function MyCertificatesPage() {
  const [data] = useState(certificates.filter(c => c.participantId === 'EMP-1042'));
    ...
    >
      <FormCard>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {data.map(cert => (
            <div key={cert.id} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', borderTop: '1px solid #e5e7eb', paddingTop: '1.5rem' }}>
              <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                <div style={{ flex: 1 }}>
                  <h3 style={{ fontSize: '1rem', fontWeight: 600, color: '#111827', margin: '0 0 0.25rem 0' }}>{cert.trainingTitle}</h3>
                  <p style={{ fontSize: '0.813rem', color: '#6b7280', margin: 0 }}>Certificate of {cert.type}</p>
                </div>
                <i className="pi pi-workspace_premium" style={{ fontSize: '2rem', color: '#f59e0b' }} />
              </div>
              
              <div style={{ background: '#f9fafb', padding: '0.75rem', borderRadius: 8, fontSize: '0.813rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  <span style={{ color: '#6b7280' }}>Cert No:</span>
                  <span style={{ fontWeight: 600, fontFamily: 'monospace' }}>{cert.certificateNo}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <Button size="small" icon="eye" variant="outlined" label="Preview" onClick={() => setPopup({ mode: 'view', item: cert })} />
                    <Button size="small" icon="download" variant="primary" label="Download PDF" />
                  </div>
                </div>
              </div>
            </div>
