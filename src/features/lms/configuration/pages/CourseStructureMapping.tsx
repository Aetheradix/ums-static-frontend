...
const BCA_NODES = [
  {
    ...
    expanded: true,
    children: [
      { key: '0-0', label: 'Module 1 : Programming Basics', icon: 'pi pi-file' },
...
export default function CourseStructureMapping() {
  ...
  const [popup, setPopup] = useState<{ visible: boolean; mode: 'add' | 'edit'; targetKey?: string; label?: string }>({ visible: false, mode: 'add' });
      ...
      <div className="flex items-center justify-between w-full pr-4 group">
        ...
        <div className="opacity-0 group-hover:opacity-100 flex gap-2 transition-opacity ml-4">
           <button 
             ...
             onClick={(e) => { e.stopPropagation(); setPopup({ visible: true, mode: 'edit', targetKey: node.key, label: node.label }); }}
             title="Edit Node"
           >
           ...
           <button 
             ...
             onClick={(e) => { e.stopPropagation(); setPopup({ visible: true, mode: 'add', targetKey: node.key, label: '' }); }}
           ...
           <button 
             ...
             onClick={(e) => {
               ...
               const deleteNode = (nodesList: any[]) => {
                 return nodesList.filter(n => n.key !== node.key).map(n => {
                   if (n.children) {
  ...
  const handlePopupSave = () => {
    ...
    ToastService.success(`Node ${popup.mode === 'edit' ? 'updated' : 'added'} successfully`);
  };

  return (
    <FormPage title="Course Structure Mapping" description="Map modules and topics to a specific course.">
        ...
        <FormCard title="Select Course">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-1">
              <DropDownList 
                label="Course" 
                textField="label" data={[{ label: 'BCA Semester 1', value: 'BCA_SEM1' }, { label: 'BBA Semester 1', value: 'BBA_SEM1' }]} 
        ...
        {selectedCourse && nodes.length > 0 && (
          <FormCard title="Structure Preview (Drag & Drop)" className="flex-1">
            <div className="mb-2 text-sm text-gray-500 flex justify-between items-center">
               <span>Hover over a node to add, edit, or delete items. You can drag and drop nodes to reorder.</span>
               <Button label="Add Root Node" size="small" variant="outlined" icon="plus" onClick={() => {
                  const newKey = nodes.length.toString();
                  setNodes([...nodes, { key: newKey, label: 'New Root Node', icon: 'pi pi-folder', children: [], expanded: true }]);
               }} />
            </div>
            ...
            <div className="flex justify-end gap-2 mt-4">
               <Button label="Save Structure" variant="primary" onClick={handleSave} />
            </div>
          </FormCard>
        )}
      </div>
      ...
      >
        <div className="flex flex-col gap-4">
          <TextBox label="Node Name" value={popup.label || ''} onChange={(val) => setPopup({ ...popup, label: val })} required />
        </div>
        <div className="flex justify-end gap-2 mt-4">
          <Button label="Cancel" variant="outlined" onClick={() => setPopup({ visible: false, mode: 'add' })} />
