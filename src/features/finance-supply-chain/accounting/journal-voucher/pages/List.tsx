...
import { JOURNAL_VOUCHERS } from '../../../mock-data';
import { useQuery } from '@tanstack/react-query';

type JVItem = (typeof JOURNAL_VOUCHERS)[0];
const QK = ['@fsc/journal-vouchers'];
function useJournalVouchersQuery() {
  const { data = [], isLoading } = useQuery({
    queryKey: QK,
    queryFn: async () => [...JOURNAL_VOUCHERS],
  });
...
export default function List() {
  const { data, isLoading } = useJournalVouchersQuery();
  ...
  return (
    <FormPage
      ...
      description="Record journal entries for non-cash adjustments and provisions."
    >
      <FormCard>
        {isLoading && <Loader />}
        <GridPanel
          data={data}
          searchBox
          searchPlaceholder="Search by voucher no, account, narration..."
          columns={[
            { cell: (_, o) => <span>{o.rowIndex + 1}</span>, width: '40px' },
            { field: 'voucherNo', header: 'Voucher No' },
            { field: 'date', header: 'Date' },
            { field: 'narration', header: 'Narration' },
            { field: 'debitAccount', header: 'Debit Account' },
            { field: 'creditAccount', header: 'Credit Account' },
            {
              field: 'amount',
              header: 'Amount (₹)',
              cell: (i: JVItem) => (
      ...
      <FormPopup
        ...
        subtitle="Record a new journal entry."
        size="lg"
      >
        <JournalVoucherForm onClose={closePopup} />
      </FormPopup>
    </FormPage>
  );
}
...
function JournalVoucherForm({ onClose }: { onClose: () => void }) {
      ...
      <div className="mb-5">
        ...
        <div className="flex items-end gap-3 flex-wrap mb-4">
          ...
          <div className="flex-1 min-w-[200px] -mb-4">
            ...
            <DropDownList
              data={[
                { label: 'Star Logistics', value: 'Star Logistics' },
                { label: 'Salary Payable', value: 'Salary Payable' },
              ]}
              textField="label"
              valueField="value"
              value={rowParticular}
