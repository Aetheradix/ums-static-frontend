...
import { PAYMENT_VOUCHERS } from '../../../mock-data';
import { useQuery } from '@tanstack/react-query';

type VoucherItem = (typeof PAYMENT_VOUCHERS)[0];
const QK = ['@fsc/payment-vouchers'];
function usePaymentVouchersQuery() {
  const { data = [], isLoading } = useQuery({
    queryKey: QK,
    queryFn: async () => [...PAYMENT_VOUCHERS],
  });
...
export default function List() {
  ...
  return (
    <FormPage
      ...
      description="Record and manage all outgoing payments."
    >
      <FormCard>
        {isLoading && <Loader />}
        <GridPanel
          data={data}
          searchBox
          searchPlaceholder="Search by voucher no, paid to, account..."
          columns={[
      ...
      <FormPopup
        ...
        size="lg"
      >
        <PaymentVoucherForm onClose={closePopup} />
      </FormPopup>
    </FormPage>
  );
}
...
function PaymentVoucherForm({ onClose }: { onClose: () => void }) {
    ...
    >
      ...
      <div className="mb-5">
        ...
        <div className="flex items-end gap-4 flex-wrap">
          ...
          {/* GST Checkbox */}
          <div className="flex items-center gap-2 h-[38px] -mb-4">
