...
const technicalPlans = [
  {
    ...
    workId: '1',
    plotArea: '2400 Sqm',
    builtUpArea: '8500 Sqm',
    floors: 'G+3',
    soilType: 'Black Cotton Soil',
    bearingCapacity: '12 T/Sqm',
    steelQty: '95 MT',
    concreteGrade: 'M20/M25',
    brickwork: '480 Cum',
    plasterArea: '8500 Sqm',
    totalMaterials: 6,
    status: 'Approved',
  },
  {
    id: '2',
    workId: '3',
    plotArea: '32000 Sqm (3.2 km road)',
    builtUpArea: '—',
    floors: 'Single Level Road',
    soilType: 'Alluvial (Medium)',
    bearingCapacity: 'CBR 4%',
    steelQty: '0 MT',
    concreteGrade: 'PCC M10',
    brickwork: '0 Cum',
    plasterArea: '0 Sqm',
    totalMaterials: 3,
    status: 'Approved',
...
export default function TechnicalPlanning() {
  ...
  const handleSave = () => {
    if (!form.workId) {
      ToastService.error('Work is required.');
      return;
    }
    setData(prev => [
      ...prev,
      { ...form, id: String(Date.now()), status: 'Draft' },
    ]);
          ...
          columns={[
            {
              ...
              cell: (t: any) => (
                ...
                >
                  {civilWorks.find(w => w.id === t.workId)?.workId}
                </span>
              ),
            },
            ...
            { field: 'steelQty', header: 'Steel (MT)' },
            {
              ...
              cell: (t: any) => (
                <span
                  className={`civil-pill ${t.status === 'Approved' ? 'green' : 'amber'}`}
                >
                  {t.status}
                </span>
              ),
        ...
        {popup.mode === 'create' ? (
          <>
            <DropDownList
              ...
              optionValue="value"
              value={form.workId}
              onChange={v => setForm((f: any) => ({ ...f, workId: v }))}
            />
            <FormGrid columns={3}>
              <TextBox
                label="Plot Area (Sqm)"
                value={form.plotArea ?? ''}
                onChange={v => setForm((f: any) => ({ ...f, plotArea: v }))}
              />
              <TextBox
                label="Built-up Area (Sqm)"
                value={form.builtUpArea ?? ''}
                onChange={v => setForm((f: any) => ({ ...f, builtUpArea: v }))}
              />
              <TextBox
                label="No. of Floors"
                placeholder="e.g. G+3"
                value={form.floors ?? ''}
                onChange={v => setForm((f: any) => ({ ...f, floors: v }))}
              />
            </FormGrid>
            <FormGrid columns={3}>
              <TextBox
                ...
                value={form.soilType ?? ''}
                onChange={v => setForm((f: any) => ({ ...f, soilType: v }))}
              />
              <TextBox
                label="Bearing Capacity (T/Sqm)"
                value={form.bearingCapacity ?? ''}
                onChange={v =>
                  setForm((f: any) => ({ ...f, bearingCapacity: v }))
                }
              />
              <TextBox
                ...
                onChange={v =>
                  setForm((f: any) => ({ ...f, concreteGrade: v }))
                }
              />
            </FormGrid>
            <FormGrid columns={2}>
              <TextBox
                label="Steel Qty (MT)"
                value={form.steelQty ?? ''}
                onChange={v => setForm((f: any) => ({ ...f, steelQty: v }))}
              />
              <TextBox
                label="Brickwork (Cum)"
                value={form.brickwork ?? ''}
                onChange={v => setForm((f: any) => ({ ...f, brickwork: v }))}
              />
            </FormGrid>
