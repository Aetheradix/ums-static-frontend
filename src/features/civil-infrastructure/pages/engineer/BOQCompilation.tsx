...
export default function BOQCompilation() {
  const [popup, setPopup] = useState<{ mode: 'closed' | 'view'; workId?: string }>({ mode: 'closed' });
  ...
  const worksList = civilWorksList.filter((w: any) => worksWithBOQ.includes(w.id));

  const workBOQ = (wid: string) => boqItems.filter(b => b.workId === wid);
  const boqTotal = (items: BOQItem[]) => items.reduce((s, i) => s + i.amount, 0);
  ...
  return (
    ...
    >
      ...
      <div style={{ background: '#fee2e2', border: '1px solid #fca5a5', borderRadius: '0.875rem', padding: '1rem 1.25rem', fontSize: '0.8125rem', color: '#991b1b', marginBottom: '1.25rem' }}>
        <strong>🔒 BOQ Lock Rule:</strong> Once AA/TS is granted and BOQ is approved, it is locked as the financial baseline. No items can be added or rates modified. Any deviation requires a Revised Estimate.
      </div>
      ...
      <FormCard>
        <GridPanel
          ...
          columns={[
            ...
            { field: 'id', header: 'BOQ Items', cell: (w: any) => <span style={{ fontWeight: 700 }}>{workBOQ(w.id).length} items</span> },
            { field: 'id', header: 'BOQ Total (₹)', cell: (w: any) => <span style={{ fontWeight: 700, color: '#16a34a' }}>₹{(boqTotal(workBOQ(w.id)) / 100000).toFixed(2)}L</span> },
            { field: 'id', header: 'BOQ Status', cell: (w: any) => {
              const b = workBOQ(w.id);
              const allLocked = b.every(item => item.isLocked);
              return allLocked ? <span className="civil-pill green">🔒 Locked Baseline</span> : <span className="civil-pill amber">Draft</span>;
            } },
            { field: 'id', header: 'Action', sortable: false, cell: (item: any) => (
              <Button size="small" label="View BOQ" icon="eye" variant="outlined"
            ...
            <>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem', marginBottom: '1rem' }}>
                {[
                  ...
                  { label: 'BOQ Total (₹)', value: `₹${(total / 100000).toFixed(2)}L` },
                  { label: 'Status', value: items.every(i => i.isLocked) ? '🔒 Locked' : 'Draft' },
                ].map(s => (
                  <div key={s.label} style={{ textAlign: 'center', padding: '0.75rem', background: '#f9fafb', borderRadius: '0.75rem', border: '1px solid #e5e7eb' }}>
                    <div style={{ fontSize: '0.65rem', fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase', marginBottom: '0.25rem' }}>{s.label}</div>
              ...
              <table className="civil-table">
                ...
                <tbody>
                  {items.map((item, idx) => (
                    <tr key={item.id}>
                      <td>{idx + 1}</td>
                      <td><span style={{ fontFamily: 'monospace', fontSize: '0.72rem', fontWeight: 700, color: '#1d4ed8' }}>{item.sorCode}</span></td>
                      <td style={{ maxWidth: 200 }}>{item.description}</td>
                      <td>{item.unit}</td>
                      <td>₹{item.govtRate.toLocaleString('en-IN')}</td>
                      <td style={{ fontWeight: 700 }}>{item.approvedQty.toLocaleString('en-IN')}</td>
                      <td style={{ fontWeight: 700, color: '#16a34a' }}>₹{(item.amount / 100000).toFixed(2)}L</td>
                      <td>{item.isLocked ? <span className="civil-pill green">🔒</span> : <span className="civil-pill amber">Draft</span>}</td>
                    </tr>
                  ))}
                  <tr style={{ background: '#f0fdf4' }}>
                    <td colSpan={6} style={{ textAlign: 'right', fontWeight: 700 }}>Grand Total</td>
                    <td style={{ fontWeight: 800, color: '#15803d' }}>₹{(total / 100000).toFixed(2)}L</td>
                    <td />
                  </tr>
                </tbody>
              </table>
