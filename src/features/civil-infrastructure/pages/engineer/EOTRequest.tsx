...
const EOT_TYPES = [
  ...
  {
    name: 'Revised Estimate (Additional Budget required)',
    value: 'Revised Estimate',
  },
];
...
export default function EOTRequest() {
  ...
  const handleSave = () => {
    if (!form.workId || !form.type || !form.reason || !form.justification) {
      ToastService.error('All required fields must be filled.');
      return;
    }
    ...
    >
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: '0.78rem', color: '#6b7280' }}>
                Delay caused by force majeure (rains, litigation, site access).
                Timeline increases but NO additional cost sanctioned.
              </div>
            </div>
          </div>
        </FormCard>
        <FormCard>
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: '0.78rem', color: '#6b7280' }}>
                Scope increase or SOR rate revision requiring additional budget
                sanction. Requires fresh AA/TS approval.
              </div>
            </div>
          </div>
        </FormCard>
      ...
      <FormCard>
        <GridPanel
            ...
            { field: 'applicationDate', header: 'Applied' },
            {
              ...
              cell: (e: EOTRequest) =>
                e.daysRequested ? (
                  ...
                  <span>
                    ₹{((e.additionalBudget ?? 0) / 100000).toFixed(1)}L
                  </span>
                ),
            },
            {
              ...
              header: 'Reason',
              cell: (e: EOTRequest) => (
                <span style={{ fontSize: '0.75rem' }}>{e.reason}</span>
              ),
            },
            {
              field: 'status',
          ...
          toolbar={
            <Button
              label="Apply EOT / Revised Estimate"
              icon="plus"
              variant="primary"
              onClick={() => {
                setForm({});
          ...
          searchBox
          searchPlaceholder="Search EOT requests..."
        />
      </FormCard>
      ...
      <FormPopup
        ...
        title={
          popup.mode === 'create'
            ? 'Apply EOT / Revised Estimate'
            : `EOT — ${popup.item?.eotNo}`
        }
              ...
              {[
                ...
                [
                  ...
                  popup.item.daysRequested
                    ? String(popup.item.daysRequested)
                    : '—',
                ],
                [
                  'Additional Budget',
                  popup.item.additionalBudget
                    ? `₹${(popup.item.additionalBudget / 100000).toFixed(1)}L`
                    : '—',
                ],
                ...
                [
                  'Approved Days',
                  popup.item.approvedDays
                    ? String(popup.item.approvedDays)
                    : '—',
                ],
