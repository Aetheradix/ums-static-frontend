...
const QUICK_ACTIONS = [
  { label: 'SOR Master', icon: 'list', path: civilUrls.sorMaster },
  {
    ...
    icon: 'file-edit',
    path: civilUrls.boqCompilation,
  },
  {
    label: 'Update Progress',
    icon: 'map-marker',
    path: civilUrls.progressMonitoring,
  },
  { label: 'E-MB Entry', icon: 'book', path: civilUrls.eMeasurementBook },
  {
    label: 'Quality Tests',
    icon: 'check-square',
    path: civilUrls.qualityTesting,
  },
    ...
    >
        ...
        <div className="civil-action-grid">
          {[
            ...
            },
            {
              ...
              icon: 'calendar-minus',
              cls: 'amber',
              path: civilUrls.milestoneSignoff,
            },
            {
              label: 'Quality Tests Pending',
            ...
            {
              label: 'MB Drafts / Submitted',
              value: pendingMBs.length,
              icon: 'book',
              cls: 'green',
              path: civilUrls.eMeasurementBook,
            },
          ].map(a => (
            <div
              key={a.label}
      ...
      <div className="civil-bottom-row">
        ...
        <FormCard
          ...
          subtitle="Works where you are Site Engineer / JE"
        >
              ...
              {myWorks.map((w: any) => {
                ...
                return (
                  <tr key={w.id}>
                    ...
                    <td>
                      <div className="civil-progress-bar-wrap">
                        <div className="civil-progress-bar-track">
                          <div
                            className={`civil-progress-bar-fill ${pct >= 75 ? 'high' : pct >= 40 ? 'medium' : 'low'}`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                        <span
                    ...
                    <td>
                      <span
                        className={`civil-pill ${w.status === 'In Progress' ? 'blue' : 'amber'}`}
                      >
                        {w.status}
                      </span>
                    </td>
              ...
              >
                ...
                >
                  <span style={{ fontWeight: 600, fontSize: '0.8125rem' }}>
                    {works.find((w: any) => w.id === log.workId)?.workId}
                  </span>
                ...
                >
                  {log.description.substring(0, 90)}...
                </div>
                <div
                  style={{
                    display: 'flex',
                ...
                >
                  <span style={{ fontSize: '0.65rem', color: '#2563eb' }}>
                    📍 {log.geoLatitude}, {log.geoLongitude}
                  </span>
