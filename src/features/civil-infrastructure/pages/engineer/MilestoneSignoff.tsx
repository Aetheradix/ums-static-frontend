...
export default function MilestoneSignoff() {
  ...
  const handleSignOff = (item: Milestone) => {
    if (item.qualityTestRequired) {
      ...
      if (anyFail) {
        ToastService.error(
          'Quality test has FAILED. Milestone cannot be closed until re-test passes.'
        );
        return;
      }
      if (anyPending) {
        ToastService.error(
          'Quality test is still PENDING. Milestone cannot be signed off until lab results are uploaded.'
        );
        return;
      }
    }
  ...
  return (
    ...
    >
        ...
        <GridPanel
          ...
          columns={[
            ...
            },
            {
              ...
              header: 'Status',
              cell: (m: Milestone) => (
                <span className={`civil-pill ${statusCls(m.status)}`}>
                  {m.status}
                </span>
              ),
            },
              ...
              cell: (item: Milestone) => (
                <div style={{ display: 'flex', gap: '0.375rem' }}>
                  ...
                  {!canSignOff(item) && item.status !== 'Completed' && (
                    ...
                    >
                      {item.qualityTestRequired &&
                      item.qualityTestStatus !== 'Pass'
                        ? '🔒 QA Pending'
                        : '⏳ Not Started'}
                    </span>
                  )}
      ...
      <FormPopup
        ...
        subtitle={
          popup.mode === 'signoff'
            ? 'Confirm sign-off. Next phase will be unlocked.'
            : 'Milestone details.'
        }
        size="lg"
      >
        {popup.item && (
          <>
            ...
            {popup.mode === 'signoff' && (
              <>
                {popup.item.qualityTestRequired &&
                  popup.item.qualityTestStatus !== 'Pass' && (
                    <div
                      style={{
                        background: '#fee2e2',
                ...
                {(!popup.item.qualityTestRequired ||
                  ...
                  <>
                    ...
                    >
                      ✅ All prerequisites met. This milestone can be signed
                      off.
                    </div>
                    <div className="flex justify-end gap-3 mt-4">
                      ...
                      />
                      <Button
                        label="Confirm Milestone Sign-off (EE)"
                        variant="primary"
                        icon="check-circle"
