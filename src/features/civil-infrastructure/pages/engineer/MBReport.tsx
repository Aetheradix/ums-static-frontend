...
export default function MBReport() {
  ...
  const reportData = works.map((w: any) => {
    const workBills = bills.filter(
      (b: any) => b.workId === w.id || b.workName === w.name
    );
    const workMBs = mbList.filter((m: any) => m.workId === w.id);

    const totalGross = workBills.reduce(
      (sum, b) => sum + (b.grossAmount || 0),
      0
    );
    const totalDeductions = workBills.reduce(
      (sum, b) =>
        sum +
        (Number(b.advanceRecovery) || 0) +
        (Number(b.securityDeposit) || 0),
      0
    );
    const totalNetPaid = workBills
    ...
    const totalAdvanceDeducted = workBills.reduce(
      (sum, b) =>
        sum + (Number(b.advanceRecovery) || Number(b.advanceAdjusted) || 0),
      0
    );

    const approvedBillsCount = workBills.filter(
      b => b.status === 'Paid' || b.status === 'Approved'
    ).length;
    const pendingBillsCount = workBills.filter(
  ...
  return (
    ...
    >
      <FormCard
        title="Work-wise Measurement & Payment Aggregates"
        subtitle="Comprehensive statement of running bills and mobilization advances."
      >
        <GridPanel
          data={reportData}
          columns={[
            ...
            { field: 'name', header: 'Work Name' },
            {
              ...
              header: 'Tender Value (₹)',
              cell: (r: any) => (
                <span>₹{(r.contractVal / 100000).toFixed(2)}L</span>
              ),
            },
            {
              ...
              cell: (r: any) => (
                <span style={{ color: '#2563eb' }}>
                  ₹{(r.totalAdvanceTaken / 100000).toFixed(2)}L
                </span>
              ),
            },
            {
              ...
              cell: (r: any) => (
                <span style={{ color: '#ef4444' }}>
                  ₹{(r.totalAdvanceDeducted / 100000).toFixed(2)}L
                </span>
              ),
            },
            {
              ...
              cell: (r: any) => (
                <span style={{ fontWeight: 700, color: '#16a34a' }}>
                  ₹{(r.totalNetPaid / 100000).toFixed(2)}L
                </span>
              ),
            },
            {
      ...
      <FormPopup
        ...
        title={
          popup.work
            ? `E-MB Ledger Summary — ${popup.work.workId}`
            : 'E-MB Statement'
        }
              ...
              <>
                ...
                >
                  {[
                    {
                      label: 'Tender Cost',
                      value: `₹${(w.contractVal / 100000).toFixed(2)}L`,
                      color: '#111827',
                    },
                    {
                      label: 'Mobilization Adv.',
                      value: `₹${(w.totalAdvanceTaken / 100000).toFixed(2)}L`,
                      color: '#2563eb',
                    },
                    {
                      label: 'Advance Recovered',
                      value: `₹${(w.totalAdvanceDeducted / 100000).toFixed(2)}L`,
                      color: '#ef4444',
                    },
                    {
                      label: 'Total Paid (Net)',
                      value: `₹${(w.totalNetPaid / 100000).toFixed(2)}L`,
                      color: '#16a34a',
                    },
                  ].map(stat => (
                    <div
                ...
                {/* MB Entries Log */}
                <FormCard
                  title="Measurement Book (MB) Entries log"
                  subtitle="Quantities executed and verified by Site Engineer."
                >
                  {w.workMBs.length > 0 ? (
                      ...
                      <tbody>
                        {w.workMBs.map((mb: any) => (
                          <tr key={mb.id}>
                            ...
                            >
                              Adv Deduct: ₹
                              {mb.advanceAdjusted?.toLocaleString('en-IN') ||
                                '0'}
                            </td>
                            <td>
                              <span
                                className={`civil-pill ${mb.status === 'Approved by EE' || mb.status === 'Verified' ? 'green' : 'blue'}`}
                              >
                                {mb.status}
                              </span>
                            </td>
                ...
                <div style={{ marginTop: '1.25rem' }}>
                  <FormCard
                    title="Running Account (RA) Bills History"
                    subtitle="Breakdown of mobilization deductions and net cleared payments."
                  >
                          ...
                          {w.workBills.map((b: any) => (
                            <tr key={b.billNo}>
                              ...
                              <td style={{ color: '#ef4444' }}>
                                -₹{b.advanceRecovery?.toLocaleString('en-IN')}
                              </td>
                              <td style={{ color: '#7c3aed' }}>
                                -₹{b.securityDeposit?.toLocaleString('en-IN')}
                              </td>
