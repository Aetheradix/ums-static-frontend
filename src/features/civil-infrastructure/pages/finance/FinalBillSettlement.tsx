...
export default function FinalBillSettlement() {
  ...
  const handleSettle = () => {
    if (!voucherNo) {
      ToastService.error('Voucher number is required.');
      return;
    }
    setSettled(prev => new Set([...prev, popup.item?.id]));
    ToastService.success(
      'Final Bill settled. Project fully closed in ERP. Contractor liability discharged.'
    );
  ...
  return (
    ...
    >
      ...
      >
        <strong>Final Bill Process:</strong> (1) All RA Bills must be settled.
        (2) Joint measurement of final quantities. (3) Reconciliation of BOQ vs.
        MB cumulative quantities. (4) Final deductions. (5) Balance payment. (6)
        ...
        <GridPanel
          ...
          columns={[
            ...
            },
            {
              ...
              cell: (w: any) =>
                w.contractAmount > 0 ? (
                  <span>₹{(w.contractAmount / 100000).toFixed(2)}L</span>
                ) : (
            ...
            },
            {
              ...
              cell: (w: any) => (
                <span style={{ fontWeight: 600 }}>
                  ₹{(w.totalPaid / 100000).toFixed(2)}L
                </span>
              ),
            },
            {
              ...
              cell: (w: any) => (
                <span
                  style={{
                    fontWeight: 700,
                    color: w.balance > 0 ? '#d97706' : '#16a34a',
                  }}
                >
                  {w.balance > 0
                    ? `₹${(w.balance / 100000).toFixed(2)}L`
                    : 'Nil'}
                </span>
              ),
            },
            ...
            },
            {
              ...
              cell: (w: any) => (
                <span
                  className={`civil-pill ${settled.has(w.id) ? 'green' : w.status === 'Ready for Final Bill' ? 'teal' : 'gray'}`}
                >
                  {settled.has(w.id) ? '✓ Settled & Closed' : w.status}
                </span>
              ),
            },
            {
              ...
              cell: (item: any) =>
                settled.has(item.id) ? (
                  ...
                  <Button
                    size="small"
                    label="Settle Final Bill"
                    icon="check-circle"
                    variant="primary"
                    onClick={() => {
                      setVoucherNo('');
                      setFinalRemarks('');
                      setPopup({ mode: 'settle', item });
                    }}
                  />
                ) : (
      ...
      >
        {popup.item && (
          <>
            ...
            >
              {[
                ...
                [
                  ...
                  `₹${(popup.item.totalPaid / 100000).toFixed(2)}L`,
                ],
                [
                  'Final Settlement Release',
                  `₹${(popup.item.balance / 100000).toFixed(2)}L`,
                ],
            ...
            >
              <strong>🔒 Settle & Close:</strong> Clicking approve releases the
              final balance payment, closes the Work ID, and marks the project
              as completed.
            </div>
            ...
            <div style={{ marginTop: '0.75rem' }}>
              <TextArea
                ...
                placeholder="Final ledger reconciliation remarks..."
                value={finalRemarks}
