...
type PopupState = { mode: 'closed' } | { mode: 'grant'; item: typeof civilWorks[0] } | { mode: 'view'; item: typeof civilWorks[0] };
...
export default function TechnicalSanction() {
  ...
  const handleGrantTS = () => {
    if (!tsAmt || Number(tsAmt) <= 0) { ToastService.error('TS Amount is required.'); return; }
    ...
    setData((prev: any[]) => prev.map((d: any) => d.id === (popup as any).item.id
      ? { ...d, tsAmount: Number(tsAmt), status: 'TS Granted' as any, tsRemarks }
      : d
    ));
    ToastService.success('Technical Sanction granted. Structural soundness certified.');
    setPopup({ mode: 'closed' }); setTsAmt(''); setTsRemarks('');
  };
  ...
  return (
    ...
    >
        ...
        <GridPanel
          ...
          columns={[
            ...
            { field: 'aaAmount', header: 'AA Amount',
              cell: (w: any) => <span>₹{(w.aaAmount / 100000).toFixed(2)}L</span> },
            { field: 'tsAmount', header: 'TS Amount (₹)',
              cell: (w: any) => w.tsAmount > 0
                ? <span style={{ fontWeight: 700, color: '#16a34a' }}>₹{(w.tsAmount / 100000).toFixed(2)}L</span>
                : <span className="civil-pill amber">Not Yet Granted</span> },
            { field: 'status', header: 'Status',
              cell: (w: any) => <StatusBadge label={w.status} variant={w.status === 'TS Granted' ? 'approved' : 'neutral'} /> },
              ...
              cell: (item: any) => (
                <div style={{ display: 'flex', gap: '0.375rem' }}>
                  ...
                  {item.status === 'AA Approved' && (
                    <Button size="small" label="Grant TS" icon="verified" variant="primary"
                      onClick={() => { setTsAmt(String(item.aaAmount * 0.995)); setTsRemarks(''); setPopup({ mode: 'grant', item }); }} />
                  )}
                </div>
              ) },
          ]}
      ...
      <FormPopup
        ...
        title={popup.mode === 'grant' ? `Grant Technical Sanction — ${(popup as any).item?.workId}` : `View — ${(popup as any).item?.workId}`}
      ...
      >
        {popup.mode !== 'closed' && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.5rem 2rem', fontSize: '0.8125rem', marginBottom: '1rem', padding: '1rem', background: '#f9fafb', borderRadius: '0.75rem' }}>
              {[
                ['Work ID', (popup as any).item.workId],
                ['AA Amount', `₹${((popup as any).item.aaAmount / 100000).toFixed(2)}L`],
            ...
            {popup.mode === 'grant' && (
              <>
                ...
                <TextArea
                  ...
                  rows={3}
                />
                <div style={{ background: '#e0f2fe', border: '1px solid #7dd3fc', borderRadius: '0.75rem', padding: '0.875rem 1rem', fontSize: '0.8125rem', color: '#0c4a6e', marginTop: '0.75rem' }}>
                   <strong>ℹ Note:</strong> Technical Sanction Amount cannot exceed the AA Amount (₹{((popup as any).item.aaAmount / 100000).toFixed(2)}L). Any cost overrun requires a Revised Estimate.
                </div>
                <div className="flex justify-end gap-3 mt-4">
                  <Button label="Cancel" variant="outlined" onClick={() => setPopup({ mode: 'closed' })} />
                  <Button label="Issue Technical Sanction" variant="primary" icon="verified" onClick={handleGrantTS} />
                </div>
              </>
            )}
          </>
