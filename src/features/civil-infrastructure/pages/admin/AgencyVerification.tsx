...
export default function AgencyVerification() {
  ...
  const handleValidateGstPan = () => {
    ...
    setTimeout(() => {
      ...
      ToastService.success(
        'GSTIN & PAN successfully validated against PWD Vendor database.'
      );
    }, 800);
  };
  ...
  const handleValidateBank = () => {
    if (!form.bankName || !form.bankAccount || !form.ifscCode) {
      ToastService.error(
        'Please enter Bank Name, Account Number, and IFSC Code.'
      );
      return;
    }
  ...
  const handleSave = () => {
    ...
    if (!form.gstNo) {
      ToastService.error('GST No is required.');
      return;
    }
    if (!form.panNo) {
      ToastService.error('PAN No is required.');
      return;
    }
    if (!form.bankAccount) {
      ToastService.error('Bank Account No is required.');
      return;
    }
    if (!gstPanValidated) {
      ToastService.error('Please validate GSTIN & PAN before onboarding.');
      return;
    }
    if (!bankMandateTracked) {
      ToastService.error('Please track/verify Bank Mandate before onboarding.');
      return;
    }
  ...
  const getLimitText = (grade?: string) => {
    ...
    if (grade === 'Class B')
      return 'Allowed project valuation limit: Up to ₹5.0 Crore.';
    if (grade === 'Class C')
      return 'Allowed project valuation limit: Up to ₹1.5 Crore.';
    if (grade === 'Class D')
      return 'Allowed project valuation limit: Up to ₹50 Lakh.';
    return '';
  };
  ...
  return (
    ...
    >
      <FormCard>
        <GridPanel
          ...
          columns={[
            ...
            {
              field: 'registeredWithPWD',
              header: 'PWD Reg.',
              cell: (c: Contractor) =>
                c.registeredWithPWD ? (
                  <span className="civil-pill green">✓ Verified</span>
                ) : (
                  <span className="civil-pill red">✗ Pending</span>
                ),
            },
            {
              field: 'securityDepositPaid',
              header: 'SD Paid',
              cell: (c: Contractor) => (
                <span>₹{(c.securityDepositPaid / 100000).toFixed(2)}L</span>
              ),
            },
            {
      ...
      <FormPopup
        ...
        title={
          popup.mode === 'create'
            ? 'Register & Onboard New Agency'
            : `Contractor Profile — ${popup.item?.companyName}`
        }
          ...
          <>
            ...
            <FormGrid columns={2}>
              ...
              <TextBox
                ...
                placeholder="e.g. Mr. Anil Mehta"
                value={form.proprietorName ?? ''}
                onChange={v => setForm(f => ({ ...f, proprietorName: v }))}
              />
            </FormGrid>
            <FormGrid columns={2}>
              <TextBox
              ...
              <TextBox
                label="Contact Email *"
                placeholder="e.g. contact@acmebuilders.com"
                value={form.email ?? ''}
            ...
            >
              2. GSTIN & PAN Validation
            </h4>
            <FormGrid columns={3}>
              <TextBox
                label="GSTIN Number *"
                placeholder="e.g. 23AABCS4832Q1ZX"
                value={form.gstNo ?? ''}
                onChange={v => setForm(f => ({ ...f, gstNo: v.toUpperCase() }))}
              />
              <TextBox
                label="PAN Number *"
                placeholder="e.g. AABCS4832Q"
                value={form.panNo ?? ''}
                onChange={v => setForm(f => ({ ...f, panNo: v.toUpperCase() }))}
              />
              <div
                style={{
                  display: 'flex',
              ...
              >
                {gstPanValidated ? (
                  ...
                  <div style={{ width: '100%' }}>
                    <Button
                      label={
                        validatingGst ? 'Validating...' : 'Verify GSTIN & PAN'
                      }
            ...
            <FormGrid columns={2}>
              ...
              >
                <span>
                  <strong>Limit Policy:</strong> {getLimitText(form.grade)}
                </span>
              </div>
            </FormGrid>
            ...
            <FormGrid columns={2}>
              <TextBox
                ...
                value={String(form.securityDepositPaid ?? '')}
                onChange={v =>
                  setForm(f => ({ ...f, securityDepositPaid: Number(v) }))
                }
              />
              <TextBox
                label="Performance Bond Value (₹)"
                value={String(form.performanceBond ?? '')}
                onChange={v =>
                  setForm(f => ({ ...f, performanceBond: Number(v) }))
                }
              />
            </FormGrid>
            ...
            <>
              ...
              >
                {[
                  ...
                  [
                    ...
                    `₹${(popup.item.securityDepositPaid / 100000).toFixed(2)}L`,
                  ],
                  [
                    'Performance Bond',
                    `₹${(popup.item.performanceBond / 100000).toFixed(2)}L`,
                  ],
                  ['Total Completed Works', String(popup.item.completedWorks)],
                  [
                    'Total Works Done (₹)',
                    `₹${(popup.item.totalWorksDone / 10000000).toFixed(2)} Cr`,
                  ],
                  [
                    'PWD Registered',
                    popup.item.registeredWithPWD ? '✓ Yes' : '✗ No',
                  ],
              ...
              >
                {[
                  ...
                  {
                    label: 'SD Received',
                    ok: popup.item.securityDepositPaid > 0,
                  },
