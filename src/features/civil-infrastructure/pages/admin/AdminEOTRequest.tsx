...
export default function AdminEOTRequest() {
  ...
  const handleProcess = (approve: boolean) => {
    ...
    const updatedData = data.map((e: any) => {
      if (e.id === item.id) {
        return {
          ...
          approvedDays:
            item.type === 'Extension of Time' && approve
              ? Number(approvedDays)
              : undefined,
          approvedBudget:
            item.type === 'Revised Estimate' && approve
    ...
    if (approve) {
      const updatedWorks = works.map((w: any) => {
        if (w.id === item.workId || w.workId === item.workId) {
          if (item.type === 'Extension of Time') {
            const originalDate = w.expectedEndDate
              ? new Date(w.expectedEndDate)
              : new Date();
            const newDate = new Date(
              originalDate.getTime() +
                Number(approvedDays) * 24 * 60 * 60 * 1000
            );
            return {
              ...w,
      ...
      ToastService.success(
        `Request approved. Project ${item.type === 'Extension of Time' ? 'schedule extended' : 'budget revised'} in system database.`
      );
    } else {
  ...
  const pendingCount = data.filter(
    (e: any) => e.status === 'Applied' || e.status === 'Under Review'
  ).length;
  const approvedEotCount = data.filter(
    (e: any) => e.status === 'Approved' && e.type === 'Extension of Time'
  ).length;
  const totalRevisedAmt = data
    .filter(
      (e: any) => e.status === 'Approved' && e.type === 'Revised Estimate'
    )
      ...
      >
        ...
        <FormCard>
          ...
          >
            ...
            <div>
              <div style={{ fontSize: '1.25rem', fontWeight: 800 }}>
                ₹{(totalRevisedAmt / 100000).toFixed(1)} Lakh
              </div>
              <div
                style={{
                  fontSize: '0.72rem',
        ...
        <GridPanel
          ...
          columns={[
            ...
            { field: 'workName', header: 'Work Name' },
            {
              ...
              cell: (e: EOTRequest) => (
                <span
                  className={`civil-pill ${e.type === 'Extension of Time' ? 'blue' : 'amber'}`}
                >
                  {e.type}
                </span>
              ),
            ...
            { field: 'applicationDate', header: 'Applied' },
            {
              ...
              cell: (e: EOTRequest) =>
                e.daysRequested ? (
                  ...
                  <span>
                    ₹{(e.additionalBudget ?? 0).toLocaleString('en-IN')}
                  </span>
                ),
            },
          ...
          searchBox
          searchPlaceholder="Search EOT applications..."
        />
      </FormCard>
          ...
          <>
            ...
            >
              {[
                ...
                [
                  'Impact Metrics',
                  popup.item.daysRequested
                    ? `${popup.item.daysRequested} days delay`
                    : `₹${(popup.item.additionalBudget ?? 0).toLocaleString('en-IN')} addition`,
                ],
            ...
            {['Applied', 'Under Review'].includes(popup.item.status) ? (
              <>
                ...
                <FormGrid columns={2}>
                  ...
                  >
                    <span>
                      Selecting "Approve" will modify contract milestones,
                      expected completion, and budget figures in active
                      registries.
                    </span>
                  </div>
                </FormGrid>
              ...
              >
                ...
                <div>
                  <span style={{ color: '#6b7280', fontSize: '0.75rem' }}>
                    EE Determination Remarks:
                  </span>
                  <div style={{ fontWeight: 600, color: '#1f2937' }}>
                    {popup.item.resolutionRemarks ?? '—'}
                  </div>
                </div>
              </div>
            )}
