...
const CATEGORIES = [
  'New Capital Construction', 'Maintenance/Overhaul', 'Renewal',
  'Strengthening', 'Deposit Work', 'Emergency Work',
].map(v => ({ name: v, value: v }));

const DEPT_OPTIONS = [
  'Civil Work', 'Electrical Work', 'Painting Work', 'Furniture Work', 'Sanitary & Plumbing Work', 'Horticulture Work'
].map(v => ({ name: v, value: v }));

const CAMPUS_OPTIONS = [
  'Main Campus', 'North Campus', 'South Campus', 'Off-Campus Centre',
].map(v => ({ name: v, value: v }));

const FUNDING_SOURCES = [
  'University Fund', 'UGC Grant', 'UGC Special Grant', 'Central Govt', 'State Govt', 'External',
].map(v => ({ name: v, value: v }));
...
const SITE_ENGINEERS = [
  'Er. Rajesh Verma', 'Er. Amit Sharma', 'Er. Priya Patel', 'Er. Sandeep Singh', 'Er. Vikram Malhotra', 'Er. Suresh Kumar', 'Er. Kavitha Menon'
].map(v => ({ name: v, value: v }));
...
const EMPTY: Partial<CivilWork> = {
  ...
  expectedEndDate: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // default 1 year
...
export default function WorkRegistration() {
  ...
  const handleSave = () => {
    if (!form.name) { ToastService.error('Work Name is required.'); return; }
    if (!form.department) { ToastService.error('Category is required.'); return; }
    if (!form.estimatedCost || form.estimatedCost <= 0) { ToastService.error('Estimated Cost must be > 0.'); return; }
    ...
    if (popup.mode === 'create') {
      setData(prev => [...prev, {
        ...
        id: String(Date.now()),
        workId: nextWorkId(),
        aaAmount: 0, tsAmount: 0, contractAmount: 0,
        physicalProgress: 0, financialProgress: 0,
        status: 'Registered',
      } as CivilWork]);
      ToastService.success('Work registered successfully. Unique Work ID generated.');
    } else if (popup.mode === 'edit') {
      setData(prev => prev.map(d => d.id === (popup as any).item.id ? ({ ...d, ...form } as CivilWork) : d));
  ...
  return (
    ...
    >
      <FormCard>
        <GridPanel
          ...
          columns={[
            ...
            { field: 'name',     header: 'Work Name', cell: (w: any) => <span style={{ fontWeight: 600 }}>{w.name}</span> },
            ...
            { field: 'estimatedCost', header: 'Est. Cost',
              cell: (w: any) => <span>₹{(w.estimatedCost / 100000).toFixed(1)}L</span> },
            { field: 'docs' as any, header: 'Uploaded Documents', cell: (w: any) => (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '2px', fontSize: '0.72rem' }}>
                {w.constructionAgreementDoc ? <span style={{ color: '#16a34a', fontWeight: 500 }}>📄 Agreement: {w.constructionAgreementDoc}</span> : null}
                {w.scopeOfWorkDoc ? <span style={{ color: '#2563eb', fontWeight: 500 }}>📄 SOW: {w.scopeOfWorkDoc}</span> : null}
                {!w.constructionAgreementDoc && !w.scopeOfWorkDoc ? <span style={{ color: '#9ca3af' }}>No Docs</span> : null}
              </div>
            ) },
            { field: 'priority', header: 'Priority',
              cell: (w: any) => (
                <span className={`civil-pill ${w.priority === 'High' ? 'red' : w.priority === 'Medium' ? 'amber' : 'gray'}`}>
                  {w.priority}
                </span>
              ) },
            { field: 'status', header: 'Status',
              cell: (w: any) => <StatusBadge label={w.status} variant={statusVariant(w.status)} /> },
          ...
          toolbar={
            <Button label="Register New Work" icon="plus" variant="primary"
              onClick={() => { setForm(EMPTY); setPopup({ mode: 'create' }); }} />
          }
          searchBox
      ...
      <FormPopup
        ...
        onHide={close}
        title={popup.mode === 'create' ? 'Register New Civil Work' : popup.mode === 'edit' ? 'Edit Work Registration' : 'View Work Registration'}
        subtitle="Fill all metadata attributes for official work registration."
        size="lg"
      >
        <FormGrid columns={2}>
          {popup.mode !== 'create' && (
            <TextBox label="Work ID" value={form.workId ?? ''} onChange={() => {}} disabled />
          )}
          <TextBox
        ...
        <FormGrid columns={3}>
          <DropDownList
            label="Category"
            data={DEPT_OPTIONS} textField={"name" as any} optionValue="value"
          ...
          <DropDownList
            label="Campus"
            data={CAMPUS_OPTIONS} textField={"name" as any} optionValue="value"
            value={form.campus}
          ...
          <DropDownList
            label="Priority"
            data={PRIORITY_OPTIONS} textField={"name" as any} optionValue="value"
            value={form.priority}
        ...
        <FormGrid columns={2}>
          ...
          <DropDownList
            ...
            data={ROUTE_OPTIONS} textField={"name" as any} optionValue="value"
        ...
        <FormGrid columns={3}>
          ...
          <DropDownList
            ...
            data={FUNDING_SOURCES} textField={"name" as any} optionValue="value"
          ...
          <DropDownList
            ...
            data={SITE_ENGINEERS} textField={"name" as any} optionValue="value"
        ...
        <FormGrid columns={2}>
          <DropDownList
            ...
            data={WORK_BASIS_OPTIONS} textField={"name" as any} optionValue="value"
        ...
        <h4 style={{ fontSize: '0.875rem', fontWeight: 700, color: '#1f2937', marginBottom: '0.5rem', borderTop: '1px solid #f3f4f6', paddingTop: '0.75rem' }}>📄 Upload Mandate Documents</h4>
        <FormGrid columns={2}>
          <div>
            ...
            <input 
              ...
              onChange={(e) => {
                ...
                if (file) {
                  ...
                  ToastService.success(`Uploaded construction agreement: ${file.name}`);
                }
              }}
              style={{
                width: '100%',
        ...
        {!isReadOnly && (
          <div className="flex justify-end gap-3 mt-4">
            ...
            <Button
              label={popup.mode === 'create' ? 'Register Work & Generate ID' : 'Update'}
