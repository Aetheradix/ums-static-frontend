...
export default function BudgetLock() {
  ...
  return (
    ...
    >
      ...
      >
        <strong>🔒 ERP Hard Stop:</strong> The system prevents tender
        publication unless an explicitly allocated and available fiscal budget
        is locked against the Work ID in this ledger. Finance must lock budget
        before the Tender module activates.
      </div>
        ...
        <GridPanel
          ...
          columns={[
            ...
            },
            {
              ...
              header: 'TS Amount',
              cell: (w: any) => (
                <span>₹{(w.tsAmount / 100000).toFixed(2)}L</span>
              ),
            },
            {
              field: 'financialYear',
              header: 'FY',
              cell: (w: any) => <span>{w.financialYear}</span>,
            },
            {
              field: 'budgetHead',
              header: 'Budget Head',
              cell: (w: any) => (
                <span style={{ fontSize: '0.75rem' }}>{w.budgetHead}</span>
              ),
            },
            {
              field: 'budgetLocked',
        ...
        {popup.mode === 'lock' && popup.item && (
          <>
            ...
            <TextArea
              ...
              placeholder="Lock reference, account code..."
              value={remarks}
              onChange={setRemarks}
              rows={2}
            />
