...
const QUICK_ACTIONS = [
  {
    label: 'Register Work',
    icon: 'plus-circle',
    path: civilUrls.workRegistration,
  },
  { label: 'AA Approval', icon: 'check-circle', path: civilUrls.adminApproval },
  {
    label: 'Technical Sanction',
    icon: 'verified',
    path: civilUrls.technicalSanction,
  },
  {
    label: 'Tender Oversight',
    icon: 'search',
    path: civilUrls.tenderOversight,
  },
  {
    label: 'Issue Work Order',
    icon: 'file-text',
    path: civilUrls.workOrderSign,
  },
  {
    label: 'Completion Certificate',
    icon: 'star',
    path: civilUrls.completionCertificate,
  },
];
  ...
  return (
    ...
    >
      ...
      <div className="civil-action-center">
        ...
        <div className="civil-action-grid">
          {[
            ...
            {
              label: 'Budget To Lock',
              value: pendingBudget,
              icon: 'wallet',
              cls: 'purple',
              path: civilUrls.budgetLock,
            },
            {
              label: 'RA Bills Pending',
              value: pendingRaBills,
              icon: 'receipt',
              cls: 'red',
              path: civilUrls.adminReports,
            },
            {
      ...
      <div className="civil-charts-row">
        {/* Budget Trend */}
        <FormCard
          title="Budget Trend (₹ Crore)"
          subtitle="Sanctioned vs. Utilized — last 6 months"
        >
      ...
      <div className="civil-bottom-row">
        ...
        <FormCard
          ...
          subtitle="Latest registered works with lifecycle status"
        >
                ...
                const statusCls =
                  w.status === 'Completed' || w.status === 'DLP Active'
                    ...
                    : w.status === 'In Progress'
                      ? 'blue'
                      : w.status === 'AA Approved' || w.status === 'TS Granted'
                        ? 'purple'
                        : w.status === 'Budget Locked'
                          ? 'teal'
                          : w.status === 'Tender Issued' ||
                              w.status === 'Tender Awarded'
                            ? 'amber'
                            : 'gray';
                return (
                  <tr key={w.id}>
                    ...
                    <td>
                      <div className="civil-progress-bar-wrap">
                        <div className="civil-progress-bar-track">
                          <div
                            className={`civil-progress-bar-fill ${fillCls}`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                        <span
            ...
            {[
              {
                ...
                value: `₹${(totalAAAmount / 10000000).toFixed(2)} Cr`,
                color: '#374151',
              },
              {
                label: 'Total Contract Value',
                value: `₹${(totalContractAmt / 10000000).toFixed(2)} Cr`,
                color: '#2563eb',
              },
              {
                label: 'RA Bills Paid (FY)',
                value: `₹${(raBills.filter(b => b.status === 'Paid').reduce((s, b) => s + b.netPayable, 0) / 100000).toFixed(1)}L`,
                color: '#16a34a',
              },
              {
                label: 'RA Bills Pending',
                value: String(pendingRaBills),
                color: '#d97706',
              },
              {
              ...
              {
                label: 'Savings vs AA',
                value: `₹${((totalAAAmount - totalContractAmt) / 100000).toFixed(1)}L`,
                color: '#16a34a',
              },
            ].map(s => (
              <div
