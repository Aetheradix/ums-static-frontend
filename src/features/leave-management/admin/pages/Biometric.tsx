  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <StatCard title="Late Entries" value={late.length} icon="schedule" colorScheme="amber" subtitle="Today" />
      </div>
      ...
      <FormCard title="Today's Biometric Records — 26 Jun 2024">
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              {['#', 'Employee', 'Department', 'Punch In', 'Punch Out', 'Working Hours', 'Status'].map(h => (
            ...
            {biometricRecords.map((rec, i) => (
              <tr key={rec.id}>
                ...
                <td style={{ padding: '0.625rem 0.75rem', borderBottom: '1px solid #f3f4f6' }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', padding: '0.125rem 0.5rem', borderRadius: 9999, fontSize: '0.688rem', fontWeight: 600, background: STATUS_STYLES[rec.status]?.bg, color: STATUS_STYLES[rec.status]?.color }}>
                    {rec.status}
                  </span>
                </td>
              </tr>
      ...
      <FormCard title="Monthly Summary — June 2024" className="mt-4">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '1rem' }}>
          {[
            ...
            <div key={s.label} style={{ padding: '1rem', border: '1px solid #f3f4f6', borderRadius: 8, background: '#f9fafb' }}>
              <p style={{ fontSize: '0.688rem', color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>{s.label}</p>
              <p style={{ fontSize: '1.25rem', fontWeight: 700, color: '#111827' }}>{s.value}</p>
            </div>
          ))}
        </div>
      </FormCard>
