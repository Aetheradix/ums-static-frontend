    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          ...
          value={`₹${totalClaim.toLocaleString()}`}
          icon="payments"
          colorScheme="teal"
          subtitle="Disbursed"
        />
        ...
        <GridPanel
          ...
          columns={[
            ...
            { field: 'destination', header: 'Destination' },
            {
              ...
              header: 'Amount',
              cell: (item: LtcRecord) => (
                <span>₹{item.claimAmount.toLocaleString()}</span>
              ),
            },
            {
              field: 'status',
      ...
      >
        <FormGrid columns={2}>
          ...
          <DropDownList
            ...
            onChange={v => setForm(f => ({ ...f, journeyType: String(v) }))}
          />
          <DatePicker
            label="Travel Date"
            value={form.travelDate}
            onChange={v => setForm(f => ({ ...f, travelDate: v as Date }))}
          />
          <TextBox
            label="Destination"
          ...
          <TextBox
            ...
            placeholder="0"
            value={String(form.familyMembers)}
            onChange={v => setForm(f => ({ ...f, familyMembers: Number(v) }))}
          />
          <TextBox
            label="Claim Amount (₹)"
            placeholder="e.g. 25000"
            value={String(form.claimAmount)}
            onChange={v => setForm(f => ({ ...f, claimAmount: Number(v) }))}
          />
        </FormGrid>
        <TextArea
          label="Purpose"
        ...
        >
          <FormGrid columns={2}>
            {[
              ...
              {
                label: 'Family Members',
                value: String(popup.item.familyMembers),
              },
              {
                label: 'Claim Amount',
                value: `₹${popup.item.claimAmount.toLocaleString()}`,
              },
