...
export default function AttendanceCalendar({
  ...
  for (let i = 0; i < firstDayOfWeek; i++)
    cells.push({ date: 0, status: 'empty' });
  for (let d = 1; d <= daysInMonth; d++) {
  ...
  return (
    <div className="att-calendar-wrapper">
      <div className="att-calendar-month-header">
        ...
        <span className="att-calendar-month-title">
      ...
      <div className="att-calendar-grid">
        {DAYS_OF_WEEK.map(d => (
          <div key={d} className="att-calendar-day-header">
            {d}
          </div>
        ))}
        {cells.map((cell, idx) => (
      ...
      <div className="att-calendar-legend">
        <span className="att-calendar-legend-item">
          <span className="att-calendar-legend-dot present" />
          Present ({present})
        </span>
        <span className="att-calendar-legend-item">
          <span className="att-calendar-legend-dot absent" />
          Absent ({absent})
        </span>
        <span className="att-calendar-legend-item">
          <span className="att-calendar-legend-dot leave" />
          Leave ({leave})
        </span>
        <span className="att-calendar-legend-item">
          <span className="att-calendar-legend-dot holiday" />
          Holiday ({holiday})
        </span>
      </div>
    </div>
