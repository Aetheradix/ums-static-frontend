...
export default function StudentLeaveRequests() {
  ...
  const handleApprove = () => {
    ...
    setPopup({ mode: 'closed' }); setRemarks('');
  };

  const handleReject = () => {
    if (popup.mode !== 'reject') return;
    if (!remarks.trim()) { ToastService.error('Remarks required for rejection.'); return; }
    ...
    >
      <FormCard>
        <GridPanel
          ...
          columns={[
            ...
            {
              field: 'attendancePct', header: 'Attendance %',
              cell: (item: LeaveApplication) => (
                <span style={{ fontWeight: 700, color: (item.attendancePct ?? 100) >= 75 ? '#16a34a' : '#ef4444' }}>
                  {item.attendancePct ?? '—'}%
                </span>
              ),
            },
            { field: 'leaveType', header: 'Leave Type' },
            {
              ...
              cell: (item: LeaveApplication) => <span>{item.days}d ({item.fromDate})</span>,
            },
            {
              field: 'status', header: 'Status',
              cell: (item: LeaveApplication) => (
                <StatusBadge label={item.status}
                  variant={item.status === 'Approved' ? 'approved' : item.status === 'Rejected' ? 'rejected' : item.status === 'Pending' ? 'pending' : 'neutral'} />
              ),
            },
            {
      ...
      {viewItem && (
        ...
        >
          ...
          <div style={{ padding: '0.75rem', background: '#f9fafb', borderRadius: 8, marginBottom: '1rem' }}>
            <p style={{ fontSize: '0.688rem', color: '#9ca3af', marginBottom: 4 }}>REASON</p>
            <p style={{ fontSize: '0.813rem', color: '#374151' }}>{viewItem.reason}</p>
          </div>
          {viewItem.attendancePct !== undefined && viewItem.attendancePct < 75 && (
            <div style={{ padding: '0.75rem', background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 8, marginBottom: '1rem' }}>
              <p style={{ fontSize: '0.813rem', color: '#b91c1c', fontWeight: 600 }}>
                ⚠ Student's attendance ({viewItem.attendancePct}%) is below 75%. Review carefully before approving.
              </p>
            </div>
          )}
      ...
      {actionItem && (
        <FormPopup
          ...
          onHide={() => setPopup({ mode: 'closed' })}
          title={popup.mode === 'approve' ? 'Approve Student Leave' : 'Reject Student Leave'}
          subtitle={`${actionItem.applicant} — ${actionItem.leaveType} (${actionItem.days} days)`}
        >
          <TextArea
            label="Remarks"
            placeholder={popup.mode === 'approve' ? 'Optional remarks for student...' : 'Reason for rejection (required)'}
            value={remarks}
            onChange={setRemarks}
            rows={3}
          />
