...
function buildApprovalQueue(): BillRow[] {
...
type PopupState =
  | { mode: 'closed' }
  | { mode: 'action'; item: BillRow; action: 'approve' | 'reject' | 'view' };

export default function List() {
  const [rows, setRows] = useState<BillRow[]>(buildApprovalQueue);
  ...
  function handleApprove(item: BillRow) {
    setRows(prev =>
  ...
  return (
    ...
    >
                    ...
                    { field: 'verifiedDate', header: 'Verified Date' },
                    {
                      ...
                      cell: (i: BillRow) => {
                        let variant:
                          ...
                          | 'rejected'
                          | 'neutral' = 'neutral';
                        if (['Approved', 'Paid'].includes(i.status))
                          variant = 'approved';
                        if (
                          ['Verified', 'Submitted', 'Received'].includes(
                            i.status
                          )
                        )
                          variant = 'pending';
      ...
      <FormPopup
        ...
        title={
          popup.mode === 'action'
            ? `${popup.action === 'approve' ? 'Approve' : 'Reject'} Bill — ${popup.item.billNo}`
            : ''
        }
        subtitle="Review bill stage and confirm your decision."
        size="lg"
      >
        {popup.mode === 'action' && (
          <div className="flex flex-col gap-5">
            <Stepper
              ...
              activeStep={Math.max(2, BILL_STAGES.indexOf(popup.item.status))}
            />
            <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-100">
              {[
                ...
                [
                  'Net Payable',
                  `₹${popup.item.amount.toLocaleString('en-IN')}`,
                ],
                ['Verified Date', popup.item.verifiedDate],
            ...
            <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
              ...
              {popup.action === 'approve' && (
                <Button
                  ...
                  variant="primary"
                  icon="check"
                  onClick={() => handleApprove(popup.item)}
                />
              )}
              {popup.action === 'reject' && (
                <Button
