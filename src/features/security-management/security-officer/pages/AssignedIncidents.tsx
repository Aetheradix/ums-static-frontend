...
export default function AssignedIncidents() {
  const [data, setData] = useState<Incident[]>(allIncidents.filter(i => i.assignedTo === OFFICER_NAME));
  const [popup, setPopup] = useState<PopupState>({ mode: 'closed' });
  const [remarks, setRemarks] = useState('');

  const close = () => { setPopup({ mode: 'closed' }); setRemarks(''); };
  ...
  const updateStatus = (newStatus: Incident['status'], action: 'investigating' | 'action_taken' | 'resolved' | 'closed') => {
    ...
    setData(prev => prev.map(d => d.id === item.id ? {
      ...d, status: newStatus,
      timeline: [...d.timeline, { action, actor: OFFICER_NAME, date: new Date().toLocaleString(), remarks }],
    } : d));
    ...
    >
      <FormCard>
        <GridPanel
            ...
            },
            {
              ...
              cell: (item: Incident) => (
                <div style={{ display: 'flex', gap: '0.375rem', flexWrap: 'wrap' }}>
                  ...
                  {item.status === 'Under Investigation' && <Button size="small" label="Action" icon="bolt" variant="primary" onClick={() => { setRemarks(''); setPopup({ mode: 'action', item }); }} />}
                  {item.status === 'Action Taken' && <Button size="small" label="Resolve" icon="check" variant="primary" onClick={() => { setRemarks(''); setPopup({ mode: 'resolve', item }); }} />}
          ...
          searchBox searchPlaceholder="Search assigned incidents..."
        />
      </FormCard>
      ...
      {viewItem && (
        ...
        >
          ...
          <div style={{ background: '#f9fafb', borderRadius: 8, padding: '0.75rem', marginBottom: '0.75rem', border: '1px solid #f3f4f6' }}>
            <p style={{ fontSize: '0.688rem', color: '#9ca3af', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Incident Description</p>
            <p style={{ fontSize: '0.813rem', color: '#374151', lineHeight: 1.6 }}>{viewItem.description}</p>
          </div>
          {viewItem.evidence && (
            <div style={{ background: '#f9fafb', borderRadius: 8, padding: '0.75rem', marginBottom: '0.75rem', border: '1px solid #f3f4f6' }}>
              <p style={{ fontSize: '0.688rem', color: '#9ca3af', marginBottom: 4, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Evidence / Attachments</p>
              <p style={{ fontSize: '0.813rem', color: '#374151' }}>{viewItem.evidence}</p>
            </div>
          )}
      ...
      {popup.mode === 'investigate' && (
        <FormPopup visible onHide={close} title="Start Investigation" subtitle={`${popup.item.incidentId} — ${popup.item.category}`}>
          <TextArea label="Investigation Notes" placeholder="Document your initial findings, witness interviews, CCTV review, etc." value={remarks} onChange={setRemarks} rows={5} />
          <div className="flex justify-end gap-3 mt-4">
            <Button label="Cancel" variant="outlined" onClick={close} />
            <Button label="Start Investigation" variant="primary" icon="search" onClick={() => updateStatus('Under Investigation', 'investigating')} />
          </div>
        </FormPopup>
      )}
      ...
      {/* Action Taken Popup */}
      {popup.mode === 'action' && (
        <FormPopup visible onHide={close} title="Update Action Taken" subtitle={`${popup.item.incidentId}`}>
          <TextArea label="Action Taken Notes" placeholder="What actions were taken? Who was notified? Steps implemented..." value={remarks} onChange={setRemarks} rows={5} />
          <div className="flex justify-end gap-3 mt-4">
            <Button label="Cancel" variant="outlined" onClick={close} />
            <Button label="Update Action" variant="primary" icon="bolt" onClick={() => updateStatus('Action Taken', 'action_taken')} />
          </div>
        </FormPopup>
      )}
      ...
      {popup.mode === 'resolve' && (
        <FormPopup visible onHide={close} title="Resolve Incident" subtitle={`${popup.item.incidentId}`}>
          <TextArea label="Resolution Notes" placeholder="How was this incident completely resolved? Describe the outcome..." value={remarks} onChange={setRemarks} rows={5} />
          <div className="flex justify-end gap-3 mt-4">
            <Button label="Cancel" variant="outlined" onClick={close} />
      ...
      {popup.mode === 'close' && (
        <FormPopup visible onHide={close} title="Close Incident" subtitle={`${popup.item.incidentId}`}>
          <TextArea label="Closure Summary" placeholder="Final summary and any corrective measures implemented..." value={remarks} onChange={setRemarks} rows={5} />
          <div className="flex justify-end gap-3 mt-4">
            <Button label="Cancel" variant="outlined" onClick={close} />
