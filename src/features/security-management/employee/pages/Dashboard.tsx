    ...
    >
        ...
        <FormCard title="My Recent Incidents">
          {myIncidents.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '2rem 1rem' }}>
              <i className="pi pi-shield" style={{ fontSize: '2rem', color: '#22c55e', marginBottom: '0.75rem', display: 'block' }} />
          ...
          <button type="button" className="sms-view-all-btn" onClick={() => navigate(smsUrls.employee.myIncidents)}>
        ...
        <FormCard title="Emergency Helplines">
          <div className="sms-list">
            {helplines.filter(h => h.status === 'Active').slice(0, 5).map(h => (
              <div key={h.id} className="sms-list-item">
                <div className="sms-list-icon" style={{ background: '#fee2e2' }}>
                  <i className="pi pi-phone" style={{ color: '#dc2626' }} />
                </div>
                <div className="sms-list-text">
                  <span className="sms-list-title">{h.helplineName}</span>
                  <span className="sms-list-sub">{h.contactNumber} · {h.availability}</span>
                </div>
              </div>
            ))}
          </div>
          <button type="button" className="sms-view-all-btn" onClick={() => navigate(smsUrls.employee.helplines)}>
            View All Helplines →
          </button>
        </FormCard>
        ...
        <FormCard title="Upcoming Programs">
          {awarenessPrograms.filter(p => p.status === 'Upcoming').map(p => (
            <div key={p.id} style={{ padding: '0.625rem', border: '1px solid #ede9fe', borderRadius: 8, marginBottom: '0.5rem', background: '#faf5ff' }}>
              ...
              <p style={{ fontSize: '0.688rem', color: '#6b7280', marginTop: 2 }}>{p.date} · {p.time} · {p.venue}</p>
            </div>
          ))}
          <button type="button" className="sms-view-all-btn" onClick={() => navigate(smsUrls.employee.awareness)}>
            View All Programs →
          </button>
        </FormCard>
      </div>
      ...
      <FormCard title="Quick Actions">
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          {[
            { label: 'Report Incident', icon: 'exclamation-triangle', path: smsUrls.employee.reportIncident, color: '#ef4444' },
            { label: 'My Incidents', icon: 'list', path: smsUrls.employee.myIncidents, color: '#3b82f6' },
            { label: 'Emergency Helplines', icon: 'phone', path: smsUrls.employee.helplines, color: '#16a34a' },
            { label: 'Safety Guidelines', icon: 'book', path: smsUrls.employee.guidelines, color: '#0891b2' },
            { label: 'Awareness Programs', icon: 'megaphone', path: smsUrls.employee.awareness, color: '#7c3aed' },
          ].map(a => (
            <button
              key={a.label}
              type="button"
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = a.color; (e.currentTarget as HTMLButtonElement).style.background = a.color + '0f'; }}
              onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = '#f3f4f6'; (e.currentTarget as HTMLButtonElement).style.background = 'white'; }}
            >
