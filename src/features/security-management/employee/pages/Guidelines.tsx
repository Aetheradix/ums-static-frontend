  ...
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '1rem' }}>
        {guidelines.filter(g => g.status === 'Active').map(g => {
          const color = CATEGORY_COLORS[g.category] ?? '#6b7280';
          const icon = CATEGORY_ICONS[g.category] ?? 'pi-book';
          return (
            <div key={g.id} style={{ border: '1px solid #f3f4f6', borderRadius: 12, padding: '1.25rem', background: 'white', boxShadow: '0 1px 3px rgba(0,0,0,0.06)', cursor: 'pointer', transition: 'all 0.15s ease' }}
            ...
            >
              ...
              <p style={{ fontSize: '0.875rem', fontWeight: 700, color: '#111827', marginBottom: '0.25rem' }}>{g.title}</p>
              <p style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '0.75rem' }}>{g.category} · {g.applicableFor}</p>
              ...
              <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.75rem' }}>
                {g.pdfUrl && <span style={{ fontSize: '0.688rem', color: '#ef4444', display: 'flex', alignItems: 'center', gap: '0.25rem' }}><i className="pi pi-file-pdf" /> PDF</span>}
                {g.videoUrl && <span style={{ fontSize: '0.688rem', color: '#2563eb', display: 'flex', alignItems: 'center', gap: '0.25rem' }}><i className="pi pi-youtube" /> Video</span>}
              </div>
            </div>
          );
        })}
