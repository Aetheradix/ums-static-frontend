...
type PopupState = { mode: 'closed' } | { mode: 'create' } | { mode: 'edit'; item: Severity } | { mode: 'view'; item: Severity };
const STATUS_OPTIONS = [{ name: 'Active', value: 'Active' }, { name: 'Inactive', value: 'Inactive' }];
const EMPTY: Partial<Severity> = { severityName: '', level: 1, color: '#22c55e', description: '', status: 'Active' };
...
export default function SeverityMaster() {
  ...
  const handleSave = () => {
    if (!form.severityName) { ToastService.error('Severity Name is required.'); return; }
    if (popup.mode === 'create') {
      setData(prev => [...prev, { ...form, id: String(Date.now()) } as Severity]);
      ToastService.success('Severity created.');
    } else if (popup.mode === 'edit') {
      setData(prev => prev.map(d => d.id === (popup as any).item.id ? { ...d, ...form } as Severity : d));
        ...
        <GridPanel
          ...
          columns={[
            ...
            { field: 'description', header: 'Description' },
            {
              ...
              cell: (item: Severity) => <StatusBadge label={item.status} variant={item.status === 'Active' ? 'approved' : 'rejected'} />,
            },
            {
          ...
          toolbar={<Button label="Add Severity" icon="plus" variant="primary" onClick={() => { setForm(EMPTY); setPopup({ mode: 'create' }); }} />}
          searchBox searchPlaceholder="Search severity levels..."
        />
      </FormCard>

      <FormPopup visible={popup.mode !== 'closed'} onHide={close}
        title={popup.mode === 'create' ? 'Add Severity' : popup.mode === 'edit' ? 'Edit Severity' : 'View Severity'}
        subtitle="Configure severity level details." size="lg"
      >
        <FormGrid columns={2}>
          <TextBox label="Severity Name" placeholder="e.g. Major" value={form.severityName ?? ''} onChange={v => setForm(f => ({ ...f, severityName: v }))} required disabled={isReadOnly} />
          <TextBox label="Level (1=Minor, 4=Critical)" placeholder="1-4" value={String(form.level ?? '')} onChange={v => setForm(f => ({ ...f, level: Number(v) }))} disabled={isReadOnly} />
        ...
        <TextArea label="Description" placeholder="Severity description" value={form.description ?? ''} onChange={v => setForm(f => ({ ...f, description: v }))} disabled={isReadOnly} rows={2} />
        {!isReadOnly && (
