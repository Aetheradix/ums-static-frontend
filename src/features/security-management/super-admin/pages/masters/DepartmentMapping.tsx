...
import { type DepartmentMapping, departmentMappings as initialData, buildings, type Building } from '../../../mocks';
import { smsUrls } from '../../../urls';

type PopupState = { mode: 'closed' } | { mode: 'create' } | { mode: 'edit'; item: DepartmentMapping } | { mode: 'view'; item: DepartmentMapping };
const STATUS_OPTIONS = [{ name: 'Active', value: 'Active' }, { name: 'Inactive', value: 'Inactive' }];
const DEPT_OPTIONS = ['Computer Science', 'Physics', 'Chemistry', 'Mathematics', 'Administration', 'Hostel Management', 'Library', 'Sports', 'Finance'].map(d => ({ name: d, value: d }));
const BUILDING_OPTIONS = buildings.map((b: Building) => ({ name: b.buildingName, value: b.buildingName }));
...
const EMPTY: Partial<DepartmentMapping> = { department: '', securityOfficer: '', building: '', contactNumber: '', status: 'Active' };
...
export default function DepartmentMappingMaster() {
  ...
  const handleSave = () => {
    if (!form.department || !form.securityOfficer) { ToastService.error('Department and Security Officer are required.'); return; }
    if (popup.mode === 'create') {
      setData(prev => [...prev, { ...form, id: String(Date.now()) } as DepartmentMapping]);
      ToastService.success('Department mapping created.');
    } else if (popup.mode === 'edit') {
      setData(prev => prev.map(d => d.id === (popup as any).item.id ? { ...d, ...form } as DepartmentMapping : d));
        ...
        <GridPanel
          ...
          columns={[
            ...
            { field: 'contactNumber', header: 'Contact' },
            {
              ...
              cell: (item: DepartmentMapping) => <StatusBadge label={item.status} variant={item.status === 'Active' ? 'approved' : 'rejected'} />,
            },
            {
          ...
          toolbar={<Button label="Add Mapping" icon="plus" variant="primary" onClick={() => { setForm(EMPTY); setPopup({ mode: 'create' }); }} />}
          searchBox searchPlaceholder="Search department mappings..."
        />
      </FormCard>

      <FormPopup visible={popup.mode !== 'closed'} onHide={close}
        title={popup.mode === 'create' ? 'Add Department Mapping' : popup.mode === 'edit' ? 'Edit Mapping' : 'View Mapping'}
        subtitle="Assign security officer and building to a department." size="lg"
      >
        <FormGrid columns={2}>
          ...
          <DropDownList label="Security Officer" data={OFFICER_OPTIONS} textField="name" optionValue="value" value={form.securityOfficer} onChange={v => setForm(f => ({ ...f, securityOfficer: v as string }))} disabled={isReadOnly} />
          ...
          <TextBox label="Contact Number" placeholder="e.g. 9876543210" value={form.contactNumber ?? ''} onChange={v => setForm(f => ({ ...f, contactNumber: v }))} disabled={isReadOnly} />
