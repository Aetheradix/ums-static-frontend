...
type PopupState =
  | { mode: 'closed' }
  | { mode: 'create' }
  | { mode: 'edit'; item: Building }
  | { mode: 'view'; item: Building };
...
export default function BuildingMaster() {
  ...
  const handleSave = () => {
    if (!form.buildingName || !form.buildingCode) {
      ToastService.error('Building Name and Code are required.');
      return;
    }
    if (popup.mode === 'create') {
      setData(prev => [
        ...prev,
        { ...form, id: String(Date.now()) } as Building,
      ]);
      ToastService.success('Building created.');
    } else if (popup.mode === 'edit') {
      setData(prev =>
        prev.map(d =>
          d.id === (popup as any).item.id ? ({ ...d, ...form } as Building) : d
        )
      );
  ...
  return (
    ...
    >
      ...
      <FormPopup
        ...
        subtitle="Configure campus building details."
        size="lg"
      >
        <FormGrid columns={2}>
          <TextBox
          ...
          <TextBox
            ...
            onChange={v => setForm(f => ({ ...f, buildingCode: v }))}
            required
            disabled={isReadOnly}
          />
          <DropDownList
          ...
          <TextBox
            ...
            onChange={v => setForm(f => ({ ...f, floors: Number(v) }))}
            disabled={isReadOnly}
          />
          <DropDownList
            label="Status"
