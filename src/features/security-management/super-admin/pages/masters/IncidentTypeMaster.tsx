...
import { type IncidentType, incidentTypes as initialData, incidentCategories } from '../../../mocks';
...
const CATEGORY_OPTIONS = incidentCategories.map(c => ({ name: c.categoryName, value: c.categoryName }));
const EMPTY: Partial<IncidentType> = { typeName: '', categoryId: '', category: '', description: '', status: 'Active' };
...
export default function IncidentTypeMaster() {
  ...
  const handleSave = () => {
    if (!form.typeName || !form.category) { ToastService.error('Type Name and Category are required.'); return; }
    if (popup.mode === 'create') {
      setData(prev => [...prev, { ...form, id: String(Date.now()) } as IncidentType]);
      ToastService.success('Incident type created.');
    } else if (popup.mode === 'edit') {
      setData(prev => prev.map(d => d.id === (popup as any).item.id ? { ...d, ...form } as IncidentType : d));
        ...
        <GridPanel
          ...
          columns={[
            ...
            { field: 'description', header: 'Description' },
            {
              ...
              cell: (item: IncidentType) => <StatusBadge label={item.status} variant={item.status === 'Active' ? 'approved' : 'rejected'} />,
            },
            {
          ...
          toolbar={<Button label="Add Incident Type" icon="plus" variant="primary" onClick={() => { setForm(EMPTY); setPopup({ mode: 'create' }); }} />}
          searchBox searchPlaceholder="Search incident types..."
        />
      </FormCard>

      <FormPopup visible={popup.mode !== 'closed'} onHide={close}
        title={popup.mode === 'create' ? 'Add Incident Type' : popup.mode === 'edit' ? 'Edit Incident Type' : 'View Incident Type'}
        subtitle="Link incident type to a category." size="lg"
      >
        <FormGrid columns={2}>
          <TextBox label="Type Name" placeholder="e.g. Unauthorized Entry" value={form.typeName ?? ''} onChange={v => setForm(f => ({ ...f, typeName: v }))} required disabled={isReadOnly} />
        ...
        <TextArea label="Description" placeholder="Brief description" value={form.description ?? ''} onChange={v => setForm(f => ({ ...f, description: v }))} disabled={isReadOnly} rows={2} />
        {!isReadOnly && (
