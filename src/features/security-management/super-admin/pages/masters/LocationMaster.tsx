...
type PopupState =
  | { mode: 'closed' }
  | { mode: 'create' }
  | { mode: 'edit'; item: Location }
  | { mode: 'view'; item: Location };
const STATUS_OPTIONS = [
  { name: 'Active', value: 'Active' },
  { name: 'Inactive', value: 'Inactive' },
];
const BUILDING_OPTIONS = buildings.map(b => ({
  name: b.buildingName,
...
export default function LocationMaster() {
  ...
  const handleSave = () => {
    if (!form.locationName || !form.building) {
      ToastService.error('Location Name and Building are required.');
      return;
    }
    if (popup.mode === 'create') {
      setData(prev => [
        ...prev,
        { ...form, id: String(Date.now()) } as Location,
      ]);
      ToastService.success('Location created.');
    } else if (popup.mode === 'edit') {
      setData(prev =>
        prev.map(d =>
          d.id === (popup as any).item.id ? ({ ...d, ...form } as Location) : d
        )
      );
  ...
  return (
    ...
    >
      ...
      <FormPopup
        ...
        subtitle="Configure location details within a building."
        size="lg"
      >
        <FormGrid columns={2}>
          <TextBox
          ...
          <DropDownList
            ...
            optionValue="value"
            value={form.floor}
            onChange={v => setForm(f => ({ ...f, floor: v as string }))}
            disabled={isReadOnly}
          />
          <DropDownList
            label="Status"
