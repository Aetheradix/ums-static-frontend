...
import {
  ...
  emergencyContactTypes as initialData,
} from '../../../mocks';
...
type PopupState =
  | { mode: 'closed' }
  | { mode: 'create' }
  | { mode: 'edit'; item: EmergencyContactType }
  | { mode: 'view'; item: EmergencyContactType };
const STATUS_OPTIONS = [
  { name: 'Active', value: 'Active' },
  { name: 'Inactive', value: 'Inactive' },
];
const EMPTY: Partial<EmergencyContactType> = {
  typeName: '',
  description: '',
  priority: 1,
  status: 'Active',
...
export default function EmergencyContactTypeMaster() {
  ...
  const handleSave = () => {
    if (!form.typeName) {
      ToastService.error('Type Name is required.');
      return;
    }
    if (popup.mode === 'create') {
      setData(prev => [
        ...prev,
        { ...form, id: String(Date.now()) } as EmergencyContactType,
      ]);
      ...
      setData(prev =>
        prev.map(d =>
          d.id === (popup as any).item.id
            ? ({ ...d, ...form } as EmergencyContactType)
            : d
        )
      );
  ...
  return (
    ...
    >
      ...
      <FormPopup
        ...
        title={
          popup.mode === 'create'
            ? 'Add Emergency Contact Type'
            : popup.mode === 'edit'
              ? 'Edit Contact Type'
              : 'View Contact Type'
        }
        subtitle="Configure emergency contact type details."
        size="lg"
      >
        <FormGrid columns={2}>
          <TextBox
        ...
        <TextArea
          label="Description"
          placeholder="Description of this contact type"
