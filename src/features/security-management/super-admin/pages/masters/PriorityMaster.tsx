...
type PopupState =
  | { mode: 'closed' }
  | { mode: 'create' }
  | { mode: 'edit'; item: Priority }
  | { mode: 'view'; item: Priority };
...
export default function PriorityMaster() {
  ...
  const handleSave = () => {
    if (!form.priorityName) {
      ToastService.error('Priority Name is required.');
      return;
    }
    if (popup.mode === 'create') {
      setData(prev => [
        ...prev,
        { ...form, id: String(Date.now()) } as Priority,
      ]);
      ToastService.success('Priority created.');
    } else if (popup.mode === 'edit') {
      setData(prev =>
        prev.map(d =>
          d.id === (popup as any).item.id ? ({ ...d, ...form } as Priority) : d
        )
      );
  ...
  return (
    ...
    >
      ...
      <FormPopup
        ...
        subtitle="Configure priority level details."
        size="lg"
      >
        <FormGrid columns={2}>
          <TextBox
            ...
            onChange={v => setForm(f => ({ ...f, priorityName: v }))}
            required
            disabled={isReadOnly}
          />
          <TextBox
            label="Level (1=Low, 4=Critical)"
            placeholder="1-4"
          ...
          <DropDownList
            ...
            optionValue="value"
            value={form.color}
            onChange={v => setForm(f => ({ ...f, color: v as string }))}
            disabled={isReadOnly}
          />
          <DropDownList
            label="Status"
