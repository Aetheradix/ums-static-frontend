...
const DEPT_OPTIONS = ['ICC Cell', 'University Health Centre', 'Safety Cell', 'Security Department', 'External', 'IT Department', 'Student Affairs'].map(d => ({ name: d, value: d }));

const EMPTY: Partial<Helpline> = {
  helplineName: '', department: '', contactNumber: '', alternateNumber: '',
  email: '', availability: '24/7', description: '', status: 'Active',
};
...
export default function HelplineManagement() {
  ...
  const handleSave = () => {
    ...
    if (popup.mode === 'create') {
      setData(prev => [...prev, { ...form, id: String(Date.now()) } as Helpline]);
      ToastService.success('Helpline added successfully.');
    } else if (popup.mode === 'edit') {
      setData(prev => prev.map(d => d.id === (popup as any).item.id ? { ...d, ...form } as Helpline : d));
    ...
    >
      <FormCard>
        <GridPanel
          ...
          toolbar={<Button label="Add Helpline" icon="plus" variant="primary" onClick={() => { setForm(EMPTY); setPopup({ mode: 'create' }); }} />}
      ...
      <FormPopup
        ...
        onHide={close}
        title={popup.mode === 'create' ? 'Add Helpline' : popup.mode === 'edit' ? 'Edit Helpline' : `${viewItem?.helplineName ?? 'Helpline'} — Details`}
        subtitle={viewItem ? `${viewItem.department} · ${viewItem.availability}` : 'Configure university helpline details.'}
        size="xl"
      >
        <FormGrid columns={2}>
          <TextBox label="Helpline Name" placeholder="e.g. Women's Helpline" value={form.helplineName ?? ''} onChange={v => setForm(f => ({ ...f, helplineName: v }))} required disabled={isReadOnly} />
          ...
          <TextBox label="Contact Number" placeholder="e.g. 1800-111-111" value={form.contactNumber ?? ''} onChange={v => setForm(f => ({ ...f, contactNumber: v }))} required disabled={isReadOnly} />
          <TextBox label="Alternate Number" placeholder="e.g. 9876500001" value={form.alternateNumber ?? ''} onChange={v => setForm(f => ({ ...f, alternateNumber: v }))} disabled={isReadOnly} />
          <TextBox label="Email" placeholder="e.g. helpline@university.edu" value={form.email ?? ''} onChange={v => setForm(f => ({ ...f, email: v }))} disabled={isReadOnly} />
          <DropDownList label="Availability" data={AVAILABILITY_OPTIONS} textField="name" optionValue="value" value={form.availability} onChange={v => setForm(f => ({ ...f, availability: v as string }))} disabled={isReadOnly} />
        ...
        <TextArea label="Description" placeholder="Brief description of this helpline" value={form.description ?? ''} onChange={v => setForm(f => ({ ...f, description: v }))} disabled={isReadOnly} rows={3} />
        {!isReadOnly && (
          <div className="flex justify-end gap-3 mt-4">
            <Button label="Cancel" variant="outlined" onClick={close} />
            <Button label={popup.mode === 'create' ? 'Add Helpline' : 'Update Helpline'} variant="primary" icon="save" onClick={handleSave} />
          </div>
        )}
      </FormPopup>
    </FormPage>
